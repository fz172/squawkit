...
class LocalAccountMigratorTest {
  ...
  fun reassign_movesWatermarkToNewUidAndScope() = runTest {
    ...
    assertThat(
      db.schemaQueries.selectWatermarksInScopePrefix(
        FROM_UID,
        "/users/$FROM_UID/%"
      )
        .awaitAsList()
    ...
    val moved =
      db.schemaQueries.selectWatermarksInScopePrefix(TO_UID, "/users/$TO_UID/%")
        .awaitAsList()
    assertThat(moved).hasSize(1)
  ...
  fun reassign_leavesOtherUsersWatermarksUntouched() = runTest {
    ...
    val other = db.schemaQueries.selectWatermarksInScopePrefix(
      "someone-else",
      "/users/someone-else/%"
    )
      .awaitAsList()
  ...
  @Test
  fun reassign_dropsWatermarkConflictsWithDestination_keepingDestinationsRank() =
    runTest {
      ...
      val destination = db.schemaQueries.selectWatermarksInScopePrefix(
        TO_UID,
        "/users/$TO_UID/%"
      )
        .awaitAsList()
