 ...
 * - `EntityScope.thingChildUnsafe("u123", "ac1")` → `/users/u123/thing/ac1/` — nested
...
data class EntityScope(val segments: List<String>) {
  ...
  companion object {
     ...
     * (see [dev.fanfly.wingslog.core.storage.ThingScopeResolver]).
     *
     * **Feature-layer per-thing code must NOT call this.** Inject [ThingScopeResolver] and use
     * `resolveNow(thingId)` / `resolve(thingId)`, which returns the caller's tree for an owned
     * thing and the host's tree for a shared one.
     *
     * Direct use is legitimate only where the owning uid is already known-correct: the resolver's
     * own implementation, and the sync engine's fan-out (which iterates own thing under
     * `userRoot` and derives shared scopes from each `SharedAircraftRef.host_uid`).
     */
    fun thingChildUnsafe(uid: String, thingId: String): EntityScope =
      EntityScope(listOf("users", uid, "thing", thingId))
  }
}
