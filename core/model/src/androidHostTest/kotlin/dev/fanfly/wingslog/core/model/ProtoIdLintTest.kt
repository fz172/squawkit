...
class ProtoIdLintTest {
  ...
  private val grandfathered = setOf(
    "Attachment", "Comment", "Component", "DeveloperSettings", "ExportRecord", "LinkedRule",
    "MaintenanceLog", "MaintenanceOverview", "MaintenanceTask", "MeterReading",
    "RequestExportDeliveryRequest", "RequestExportDeliveryResponse", "SharedAircraftRef",
    "Squawk", "Technician", "Thing", "ThingTemplate", "UserInfo",
  )
  ...
  private val idFieldNames = Regex("""^\s*(?:repeated\s+)?string\s+(id|[a-z0-9_]+_id)\s*=""", RegexOption.MULTILINE)
  ...
  fun everyIdMessageIsFrozenAtOneValueField() {
    ...
    assertWithMessage("id/ids.proto declares no messages").that(messages).isNotEmpty()
    messages.forEach { (name, body) ->
      val fields = Regex("""^\s*\S+\s+([a-z0-9_]+)\s*=\s*(\d+);""", RegexOption.MULTILINE)
        .findAll(body).map { it.groupValues[1] to it.groupValues[2] }.toList()
      assertWithMessage("$name must be exactly `string value = 1`")
        .that(fields).containsExactly("value" to "1")
      assertWithMessage("$name.value must be a string").that(body).contains("string value = 1;")
    }
  }
  ...
  private fun messages(proto: String): List<Pair<String, String>> =
    Regex("""^message\s+(\w+)\s*\{(.*?)^\}""", setOf(RegexOption.MULTILINE, RegexOption.DOT_MATCHES_ALL))
      .findAll(proto).map { it.groupValues[1] to it.groupValues[2] }.toList()

  private fun protoFiles(): List<File> =
    protoRoot().walkTopDown().filter { it.isFile && it.extension == "proto" }.toList()

  private fun protoRoot(): File = File(repoRoot(), "core/model/src/commonMain/proto")
