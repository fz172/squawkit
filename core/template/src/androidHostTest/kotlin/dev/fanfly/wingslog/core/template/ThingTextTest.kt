 ...
 *
 * The bug: a make saved as "Volar " rendered "Volar  T2i" everywhere the two are joined — the
...
class ThingTextTest {
  ...
  fun aBlankPartIsDroppedRatherThanJoined() {
    assertThat(listOf("Volar", "").joinAsPhrase()).isEqualTo("Volar")
    assertThat(listOf("", "  ", "T2i").joinAsPhrase()).isEqualTo("T2i")
