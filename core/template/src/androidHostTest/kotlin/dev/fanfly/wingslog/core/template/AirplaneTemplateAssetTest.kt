...
/**
 * That the committed `airplane.v11.pb` is what the app thinks it is (#675).
 *
 * **Why a committed binary is checked by a test rather than compiled in Gradle.** Wire has no
 * protobuf text-format parser, so turning `airplane.v11.textproto` into bytes needs `protoc`, which
...
class AirplaneTemplateAssetTest {
  ...
  private val asset: File
    get() = File(repoRoot(), "core/template/templates/binary/airplane.v11.pb")
   ...
   *
   * Not a tautology, though both sides trace to one file: the left is `airplane.v11.pb` read from
