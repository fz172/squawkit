...
/**
 * That the committed `airplane.v13.pb` is what the app thinks it is (#675).
 *
 * **Why a committed binary is checked by a test rather than compiled in Gradle.** Wire has no
 * protobuf text-format parser, so turning `airplane.v13.textproto` into bytes needs `protoc`, which
...
class AirplaneTemplateAssetTest {
  ...
  private val asset: File
    get() = File(repoRoot(), "core/template/templates/binary/airplane.v13.pb")
   ...
   *
   * Not a tautology, though both sides trace to one file: the left is `airplane.v13.pb` read from
  ...
  fun theLexiconStillSaysWhatTheAppSays() {
    ...
    assertThat(lexicon.down_alert_hint).isEqualTo("AOG squawks must be resolved before flight")
  ...
  fun theEmptyStateCopyStillSaysWhatTheAppSays() {
    ...
    assertThat(empty.log_onboarding_hint).isEqualTo(
      "Log your current airframe, engine, and prop times to start tracking maintenance " +
        "intervals accurately. This will serve as your logbook's baseline.",
    )
  }
  ...
  fun theStructureTheAirplaneScreensAssumeIsIntact() {
    ...
    assertThat(AirplaneTemplate.AIRPLANE_METERS.map { it.component_slot_key })
      ...
      .inOrder()
    // The propeller turns whenever the airframe flies, so the log form offers its reading. The
    // engine's empty string is the load-bearing half: its hours come off its own tach, and a
    // figure derived from the airframe would be offered on every log.
    assertThat(AirplaneTemplate.AIRPLANE_METERS.map { it.follows_meter_key })
      .containsExactly("", "", "airframe_hours")
      .inOrder()
