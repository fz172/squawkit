...
class SyncCursorStoreTest {
  ...
  fun get_noCursorRow_returnsNull() {
    val cursor = store.get(TEST_UID, TEST_KIND, TEST_SCOPE)
  ...
  fun markHydrated_createsCursorWithHydratedTrue() {
    store.markHydrated(TEST_UID, TEST_KIND, TEST_SCOPE, lastSeenRemote = 5000L)
  ...
  fun markHydrated_withNullLastSeen_storesNullLastSeenRemote() {
    store.markHydrated(TEST_UID, TEST_KIND, TEST_SCOPE, lastSeenRemote = null)
  ...
  fun recordFailure_firstAttempt_failedAttemptsIsOne() {
    store.recordFailure(TEST_UID, TEST_KIND, TEST_SCOPE)
  ...
  fun recordFailure_afterHydrated_hydratedStaysTrue() {
    store.markHydrated(TEST_UID, TEST_KIND, TEST_SCOPE, lastSeenRemote = 1000L)
    store.recordFailure(TEST_UID, TEST_KIND, TEST_SCOPE)
  ...
  fun advanceLastSeen_noCursorRow_createsCursorWithGivenTs() {
    store.advanceLastSeen(TEST_UID, TEST_KIND, TEST_SCOPE, remoteTs = 3000L)
  ...
  fun advanceLastSeen_newerTs_updatesLastSeenRemote() {
    store.advanceLastSeen(TEST_UID, TEST_KIND, TEST_SCOPE, remoteTs = 1000L)
    store.advanceLastSeen(TEST_UID, TEST_KIND, TEST_SCOPE, remoteTs = 5000L)
  ...
  fun cursors_differentScopesSameKind_areIsolated() {
    val scopeOther = EntityScope.aircraftChild(TEST_UID, "other-aircraft")
    store.markHydrated(TEST_UID, TEST_KIND, TEST_SCOPE, lastSeenRemote = 1000L)

    val cursorOther = store.get(TEST_UID, TEST_KIND, scopeOther)
  ...
  fun cursors_differentKindsSameScope_areIsolated() {
    ...
    val cursorOther = store.get(TEST_UID, CollectionKind.Aircraft, TEST_SCOPE)
    assertThat(cursorOther).isNull()
  }
}
