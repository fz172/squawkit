...
private const val TEST_UID = "user-hydration-001"
private const val TEST_AIRCRAFT_ID = "aircraft-hydration-001"
private val TEST_SCOPE = EntityScope.aircraftChild(TEST_UID, TEST_AIRCRAFT_ID)
...
class HydrationRunnerTest {
  ...
  fun setUp() {
    ...
    runner = HydrationRunner(db = db, fetcher = fetcher, cursors = cursors)
  }
  ...
  fun runFor_success_insertsAllDocsWithDirtyFalse() = runTest {
    val docs = listOf(
      buildRemoteEntity(id = "log-1", remoteTsMs = 1000L),
      buildRemoteEntity(id = "log-2", remoteTsMs = 2000L),
      buildRemoteEntity(id = "log-3", remoteTsMs = 3000L),
    )
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) } returns docs

    val result = runner.runFor(TEST_UID, TEST_KIND, TEST_SCOPE)

    assertThat(result).isTrue()
    for (doc in docs) {
      val row = db.schemaQueries.selectOneForSync(TEST_KIND, TEST_SCOPE.toPath(), doc.id)
        .executeAsOneOrNull()
  ...
  fun runFor_success_marksCursorHydratedTrue() = runTest {
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) } returns listOf(
      buildRemoteEntity(id = "log-a", remoteTsMs = 5000L),
    )

    runner.runFor(TEST_UID, TEST_KIND, TEST_SCOPE)

    val cursor = cursors.get(TEST_UID, TEST_KIND, TEST_SCOPE)
  ...
  fun runFor_success_cursorLastSeenRemoteIsMaxTs() = runTest {
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) } returns listOf(
      buildRemoteEntity(id = "log-x", remoteTsMs = 1000L),
      buildRemoteEntity(id = "log-y", remoteTsMs = 9000L),
      buildRemoteEntity(id = "log-z", remoteTsMs = 4000L),
    )
  ...
  fun runFor_emptyCollection_returnsTrueAndCursorHydratedWithNullLastSeen() = runTest {
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) } returns emptyList()
  ...
  fun runFor_fetcherThrows_returnsFalse() = runTest {
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) } throws RuntimeException("network error")
  ...
  fun runFor_fetcherThrows_incrementsFailedAttempts() = runTest {
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) } throws RuntimeException("timeout")
  ...
  fun runFor_fetcherThrowsTwice_failedAttemptsAccumulates() = runTest {
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) } throws RuntimeException("repeated failure")
  ...
  fun runFor_retryAfterFailureSucceeds_resetsFailedAttempts() = runTest {
    val docs = listOf(buildRemoteEntity(id = "log-retry", remoteTsMs = 1000L))
    coEvery { fetcher.fetchAll(TEST_KIND, TEST_SCOPE) }
      .throws(RuntimeException("first attempt fails"))
      .andThen(docs)

    runner.runFor(TEST_UID, TEST_KIND, TEST_SCOPE) // fails
    val result = runner.runFor(TEST_UID, TEST_KIND, TEST_SCOPE) // succeeds
