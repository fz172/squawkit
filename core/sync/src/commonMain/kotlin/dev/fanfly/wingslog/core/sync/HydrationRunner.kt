...
class HydrationRunner(
   ...
   */
  suspend fun runFor(uid: String, kind: CollectionKind, scope: EntityScope): Boolean {
    return runCatching {
      val docs = fetcher.fetchAll(kind, scope)
      val maxTs = applyAndComputeMaxTs(kind, scope, docs)
      cursors.markHydrated(uid = uid, kind = kind, scope = scope, lastSeenRemote = maxTs)
      ...
      cursors.recordFailure(uid, kind, scope)
      false
    }
  }
