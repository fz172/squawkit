...
class SyncCursorStore(private val db: WingsLogDatabase) {
  ...
  fun get(uid: String, kind: CollectionKind, scope: EntityScope): SyncCursor? =
    db.schemaQueries.selectCursor(uid, kind, scope.toPath()).executeAsOneOrNull()?.toCursor(scope)

  fun markHydrated(uid: String, kind: CollectionKind, scope: EntityScope, lastSeenRemote: Long?) {
  ...
  fun advanceLastSeen(uid: String, kind: CollectionKind, scope: EntityScope, remoteTs: Long) {
    val current = get(uid, kind, scope)
  ...
  fun recordFailure(uid: String, kind: CollectionKind, scope: EntityScope) {
    val current = get(uid, kind, scope)
    db.schemaQueries.upsertCursor(
      uid = uid,
      collection = kind,
