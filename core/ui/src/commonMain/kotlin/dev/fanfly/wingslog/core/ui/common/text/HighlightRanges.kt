...
fun highlightRanges(text: String, words: Set<String>): List<IntRange> {
  ...
  val lower =
    CharArray(text.length) { text[it].lowercaseChar() }.concatToString()
  val ranges = ArrayList<IntRange>()
  var i = 0
  while (i < lower.length) {
    if (!lower[i].isTokenChar()) {
      i++; continue
    }
    var end = i
    while (end < lower.length && lower[end].isTokenChar()) end++
    val run = lower.substring(i, end)
      .trim('.', '/', '-')
    val runStart = i + (end - i - lower.substring(i, end)
      .trimStart('.', '/', '-').length)
    if (run.isNotEmpty() && run in words) {
      ...
      for (k in i..end) {
        if (k == end || lower[k] in INNER) {
          if (k > partStart && lower.substring(
              partStart,
              k
            ) in words
          ) ranges.add(partStart until k)
          partStart = k + 1
        }
      }
    }
  ...
  for (phrase in words) {
    ...
    while (from >= 0) {
      ...
      val bounded =
        (from == 0 || !lower[from - 1].isLetterOrDigit()) && (to == lower.length || !lower[to].isLetterOrDigit())
