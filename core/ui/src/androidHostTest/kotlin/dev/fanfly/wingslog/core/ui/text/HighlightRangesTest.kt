...
class HighlightRangesTest {
  ...
  private fun marked(text: String, words: Set<String>): String {
    ...
    for (r in ranges) {
      sb.append(text, cursor, r.first)
        .append('[')
        .append(text, r.first, r.last + 1)
        .append(']')
      cursor = r.last + 1
    }
    return sb.append(text.substring(cursor))
      .toString()
  }
  ...
  fun wholeWordsOnly_caseInsensitive() {
    assertThat(
      marked(
        "Transponder check; checked the transponders.",
        setOf("transponder", "check")
      )
    )
  ...
  fun referencesWholeOrByPart() {
    assertThat(
      marked(
        "Tested per 91.413 and 91.217.",
        setOf("91.413")
      )
    ).isEqualTo("Tested per [91.413] and 91.217.")
    assertThat(marked("Tested per 91.413 and 91.217.", setOf("413"))).isEqualTo(
      "Tested per 91.[413] and 91.217."
    )
    assertThat(marked("AD 2011-10-09 complied", setOf("2011-10-09"))).isEqualTo(
      "AD [2011-10-09] complied"
    )
  }
  ...
  fun phrasesAndNothing() {
    assertThat(
      marked(
        "Replaced emergency locator transmitter battery",
        setOf("emergency locator transmitter")
      )
    )
