...
/**
 * Manages thing-share access: members, invites, roles, and the technician mirror.
 *
...
interface SharingManager {
   ...
   * the thing was shared in), so it is instant and offline-correct; false for an own thing and
   * while signed out.
   *
   * This is what makes the attachment gate thing-scoped (design §9.7): on a foreign host's
   * thing the member is never blocked by their *own* subscription — the host's entitlement governs
  ...
  /**
   * Joins by code alone — no thing id, no host uid. The invitee never holds either, which is the
   * whole point: those are what an attacker needed to fabricate a same-id thing (#202/#204).
   */
  suspend fun redeemInvite(code: String): Result<RedeemOutcome>

  /**
   * Resolves a code to what the invitee needs to decide — thing label, inviting owner, role — and
   * grants nothing (#201). Impossible before #164: an invitee held a real thing id, and the rules
   ...
   * `SharedAircraftRef` is still in flight. Pass [alsoPublishTo] with that thing id (the redeem
  ...
  /**
   * The same, scoped to one thing: the members of *this* share who have published a mirror. This
   * is what the log-form picker lists — you sign a log for a specific thing, so only that
