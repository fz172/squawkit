...
class RedeemViewModelTest {
  ...
  fun setUp() {
    ...
    coEvery { sharing.publishTechnicianMirror(any()) } returns Result.success(
      Unit
    )
    // The sheet previews what you are joining (#201); irrelevant to these assertions unless stated.
    coEvery { sharing.previewInvite(any()) } returns Result.failure(
      RuntimeException("no preview")
    )
  ...
  private fun viewModel() =
    RedeemViewModel(sharingManager = sharing, auth = auth)
