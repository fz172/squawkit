...
class TaskStatusManagerImplTest {
  ...
  private val done = MaintenanceTask(
    id = "done",
    title = "One-time, complied",
    is_one_time = true
  )
  ...
  fun setUp() {
    ...
    every { taskStore.observeAll(scope) } returns flowOf(
      listOf(
        ...
        later,
        soon
      ).map { StorageEntity(it.id, it, Instant.DISTANT_PAST) })
    every { logStore.observeAll(scope) } returns flowOf(emptyList())
    every { dueManager.computeNextDue(soon, any(), any()) } returns DueMetadata(
      nextDueDate = LocalDate(2026, 10, 1)
    )
    every {
      dueManager.computeNextDue(
        later,
        any(),
        any()
      )
    } returns DueMetadata(nextDueDate = LocalDate(2027, 3, 1))
    every { dueManager.computeNextDue(done, any(), any()) } returns DueMetadata(
  ...
  fun activeFirstByUrgency_thenComplied() = runTest {
    val result = manager().observeTasksWithStatus(THING_ID)
      .first()
    assertThat(result.map { it.card.id }).containsExactly(
      "soon",
      "later",
      "done"
    )
  ...
  fun noScope_emitsEmpty() = runTest {
    ...
    assertThat(
      manager().observeTasksWithStatus(THING_ID)
        .first()
    ).isEmpty()
  }
  ...
  fun logsReachTheDueManager() = runTest {
    ...
    } returns DueMetadata(nextDueDate = LocalDate(2027, 12, 1))
    val result = manager().observeTasksWithStatus(THING_ID)
      .first()
    assertThat(result.map { it.card.id }).containsExactly(
      "later",
