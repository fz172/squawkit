...
class ExportViewModelDeliveryTest {
  ...
  @Test
  fun `granted email export resolves delivery info and is not locked`() =
    runTest(dispatcher) {
      ...
      assertThat(state.resolvedDeliveryInfo?.source).isEqualTo(
  ...
  @Test
  fun `eligible user with gate off surfaces shown-locked email option`() =
    runTest(dispatcher) {
      authState.value = user(anonymous = false, email = "pilot@example.com")
