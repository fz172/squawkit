...
class LogbookExportArchiveBuilderTest {
  ...
  fun buildEntries_embedsAttachmentPayloadsAndLinksCsvRows() {
    ...
    assertThat(entries["csv/01_Airframe.csv"]?.bytes?.decodeToString())
    ...
    assertThat(entries["csv/00_Aircraft_Info.csv"]?.bytes?.decodeToString())
      .contains("Export App Version,Hopply 1.0.260519.10 (364)")
    assertThat(entries["README.txt"]?.bytes?.decodeToString())
      .contains("App:       Hopply 1.0.260519.10 (364)")
    assertThat(entries["csv/01_Airframe.csv"]?.bytes?.decodeToString())
      .doesNotContain("Log ID")
    assertThat(entries["csv/10_Compliance.csv"]?.bytes?.decodeToString())
      .doesNotContain("Task ID")
    assertThat(entries["csv/11_Squawks.csv"]?.bytes?.decodeToString())
      .doesNotContain("Squawk ID")
    assertThat(entries["csv/20_Technicians.csv"]?.bytes?.decodeToString())
