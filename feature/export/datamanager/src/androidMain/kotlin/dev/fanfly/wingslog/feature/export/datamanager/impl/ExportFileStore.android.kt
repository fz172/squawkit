...
actual class ExportFileStore(private val context: Context) {
    ...
    withContext(Dispatchers.IO) {
      ...
      val pendingValues = ContentValues().apply {
        ...
        put(
          ...
          "${Environment.DIRECTORY_DOWNLOADS}/SquawkIt"
        )
  ...
  private fun discoverArchives(): List<LocalArchiveRecord> {
    ...
    val selectionArgs =
      arrayOf("%${Environment.DIRECTORY_DOWNLOADS}/SquawkIt/%")
      ...
      ?.use { cursor ->
        ...
        while (cursor.moveToNext()) {
          ...
          records += LocalArchiveRecord(
            filePath = ContentUris.withAppendedId(collection, id)
              .toString(),
  ...
  private fun readIndex(ownerUid: String): List<ExportRecord> =
    runCatching {
      indexFile(ownerUid).takeIf { it.exists() }
        ?.readBytes()
    }
      .getOrNull()
