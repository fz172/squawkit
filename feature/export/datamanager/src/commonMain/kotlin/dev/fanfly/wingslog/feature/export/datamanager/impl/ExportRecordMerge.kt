...
internal object ExportRecordMerge {
  fun merge(local: List<ExportRecord>, remote: List<ExportRecord>): List<ExportRecord> {
    ...
    return (localById.keys + remoteById.keys).mapNotNull { exportId ->
      ...
      when {
        localRecord != null && remoteRecord != null -> localRecord.withRemote(remoteRecord)
  ...
  private fun ExportRecord.withRemote(remote: ExportRecord) = copy(
    ...
    remote_expires_at_epoch_millis =
      remote.remote_expires_at_epoch_millis.takeIf { it > 0L } ?: remote_expires_at_epoch_millis,
  )
}
