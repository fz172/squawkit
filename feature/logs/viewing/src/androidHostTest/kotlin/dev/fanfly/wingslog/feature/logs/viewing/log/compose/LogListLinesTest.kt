...
class LogListLinesTest {
  ...
  fun headsEachMonthWithItsCount() {
    ...
    assertThat(lines.filterIsInstance<LogListLine.MonthHeader>()).containsExactly(
      LogListLine.MonthHeader(LocalDate(2026, 9, 1), 2),
      LogListLine.MonthHeader(LocalDate(2026, 7, 1), 1),
    ).inOrder()
  }
  ...
  fun anUndatedLogHeadsItsOwnGroup() {
    ...
    assertThat(lines[2]).isEqualTo(LogListLine.MonthHeader(month = null, count = 1))
  }
