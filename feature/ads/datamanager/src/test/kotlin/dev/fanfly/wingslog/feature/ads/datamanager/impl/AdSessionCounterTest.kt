...
class AdSessionCounterTest {
  ...
  fun `grants up to the cap and then nothing`() {
    startSession()
    repeat(AdSessionCounter.CAP) { assertThat(counter.reserve(nextKey(), 1)).isEqualTo(1) }
  ...
  fun `reserving zero or fewer units is a programming error`() {
    ...
    assertThat(runCatching { counter.reserve(nextKey(), -1) }.isFailure).isTrue()
  }
  ...
  fun `the budget is spent globally, not per surface`() {
    ...
    startSession()
    val granted = counter.reserve(nextKey(), 2) + counter.reserve(nextKey(), 2) + counter.reserve(nextKey(), 2)
  ...
  fun `a slot already granted still renders after the cap is reached`() {
    ...
    repeat(AdSessionCounter.CAP - 1) { counter.reserve(AdSlotKey(AdSurface.TASKS, it), 1) }
    assertThat(counter.headroom).isEqualTo(0)
