...
actual fun sha256Hex(bytes: ByteArray): String {
  ...
  return intArrayOf(h0, h1, h2, h3, h4, h5, h6, h7)
    .joinToString(separator = "") { it.toHexWord() }
}

private fun ByteArray.sha256Padded(): ByteArray {
  ...
  for (i in 0 until 8) {
    padded[paddedLength - 1 - i] = ((bitLength ushr (i * 8)) and 0xff).toByte()
  }
  return padded
}
...
private fun Int.toHexWord(): String {
  val chars = CharArray(8)
  for (i in 0 until 8) {
    val nibble = (this ushr ((7 - i) * 4)) and 0x0f
    chars[i] = "0123456789abcdef"[nibble]
  }
