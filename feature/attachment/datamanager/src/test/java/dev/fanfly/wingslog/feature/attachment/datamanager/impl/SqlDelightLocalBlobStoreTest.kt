...
class SqlDelightLocalBlobStoreTest {
  ...
  @Before
  fun setUp() {
    rootDir = Files.createTempDirectory("blob-test")
      .toFile()
  ...
  @Test
  fun upsertRemoteOnly_does_not_overwrite_an_existing_synced_row() =
    runTest(ioContext) {
      val id = BlobId("att-3")
  ...
  @Test
  fun upload_happy_path_transitions_LocalOnly_to_Uploading_to_Synced() =
    runTest(ioContext) {
      val id = BlobId("att-4")
  ...
  @Test
  fun markFailedTransient_reverts_to_LocalOnly_and_increments_attempts() =
    runTest(ioContext) {
      val id = BlobId("att-5")
      ...
      val raw = db.schemaQueries.selectBlobById(id.value)
        .executeAsOne()
  ...
  @Test
  fun invalid_transition_markUploaded_from_LocalOnly_throws() =
    runTest(ioContext) {
      val id = BlobId("att-6")
      store.put(id, "y".toByteArray(), null, scopeA)
  ...
  @Test
  fun invalid_transition_markUploading_from_RemoteOnly_throws() =
    runTest(ioContext) {
      val id = BlobId("att-7")
  ...
  @Test
  fun installDownloaded_with_matching_sha_flips_to_Synced() =
    runTest(ioContext) {
      val id = BlobId("att-8")
  ...
  @Test
  fun installDownloaded_with_mismatched_sha_returns_failure_and_writes_nothing() =
    runTest(ioContext) {
      val id = BlobId("att-9")
  ...
  @Test
  fun delete_after_synced_removes_file_and_tombstones_row() =
    runTest(ioContext) {
  ...
  fun delete_of_unknown_id_is_noop() = runTest(ioContext) {
    ...
    // No exception, no row.
    assertThat(
      db.schemaQueries.selectBlobById("ghost")
        .executeAsOneOrNull()
    ).isNull()
  }
  ...
  fun rows_in_one_scope_do_not_leak_into_another() = runTest(ioContext) {
    ...
    val rowA = db.schemaQueries.selectBlobById("a-only")
      .executeAsOne()
    val rowB = db.schemaQueries.selectBlobById("b-only")
      .executeAsOne()
    assertThat(rowA.scope_path).isEqualTo("/users/user-A/")
    assertThat(rowB.scope_path).isEqualTo("/users/user-B/")

    val sumA = db.schemaQueries.sumBlobSizeInScope("/users/user-A/")
      .executeAsOne()
    val sumB = db.schemaQueries.sumBlobSizeInScope("/users/user-B/")
      .executeAsOne()
  ...
  fun observe_unknown_id_emits_null() = runTest(ioContext) {
    val ref = store.observe(BlobId("nope"))
      .first()
