 ...
 *
 * The mapping is mechanical: an [dev.fanfly.wingslog.core.storage.EntityScope] is a sequence of `(collection, doc)` segments, and a
 * [dev.fanfly.wingslog.core.storage.CollectionKind] is appended as the leaf collection identified by [dev.fanfly.wingslog.core.storage.CollectionKind.wireName]. The
 * document id is the same id used in the local [entity] table.
 *
 * Examples
 * - `Thing` at `userRoot("u1")` with id `ac1` → `/users/u1/thing/ac1`
 * - `MaintenanceLog` at `thingChildUnsafe("u1", "ac1")` with id `l9` →
