 ...
 *
 * Every variant names an [aircraftId], because every notification this app sends is about one
 * aircraft — so selecting it is the one thing every tap does, whatever else follows.
 *
...
sealed interface NotificationTapTarget {
  ...
  data class Aircraft(override val aircraftId: String, val tab: String? = null) :
    NotificationTapTarget

  data class Squawk(override val aircraftId: String, val squawkId: String) :
    NotificationTapTarget

  data class Task(override val aircraftId: String, val taskId: String) :
    NotificationTapTarget

  data class Log(override val aircraftId: String, val logId: String) :
    NotificationTapTarget
}
