...
class RecordFilterFlowsTest {
  ...
  fun typingIsDebounced_structureAndClearAreImmediate() = runTest {
    ...
    val job = launch { source.debouncedQuery(150).collect { seen += it } }
    advanceTimeBy(1)
    ...
    source.value = RecordFilter(query = "tr", components = setOf(ComponentType.COMPONENT_ENGINE))
    advanceTimeBy(1)
    assertThat(seen.last()).isEqualTo(source.value)

    source.value = RecordFilter(components = setOf(ComponentType.COMPONENT_ENGINE))
    advanceTimeBy(1)
