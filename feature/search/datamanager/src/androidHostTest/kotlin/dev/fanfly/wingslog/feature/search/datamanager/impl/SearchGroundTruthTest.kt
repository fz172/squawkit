...
class SearchGroundTruthTest {
  ...
  private data class Record(val id: String, val title: String, val body: String = "", val ref: String = "")
  ...
  private val logbook = listOf(
    Record("gasket", "Replaced left magneto base gasket", "Torqued to spec, ground run leak check satisfactory."),
    Record("xpdr", "Installed Garmin GTX 335", "Removed KT-76A transponder, serial 3AB012345. Tested per 91.413 and 91.217."),
    Record("oil", "Oil and filter change", "Phillips X/C 20W-50, 7 qt. Filter cut open, no metal found."),
    Record("strut", "Serviced nose strut", "MIL-H-5606 and nitrogen to 45 psi."),
    Record("elt", "ELT battery", "Replaced emergency locator transmitter battery pack, ACK E-04. Self-test satisfactory."),
    Record("annual", "Annual inspection", "Completed IAW Cessna 172S MM. Aircraft returned to service.", ref = "AD 2011-10-09"),
    Record("alternator", "Alternator field wire repair", "Chafed at firewall pass-through. Repaired and secured."),
    Record("spinner", "Spinner screw", "Replaced missing prop spinner bulkhead screw and safetied."),
    Record("certs", "IFR certification", "Transponder and altimeter certification per 91.411 and 91.413.", ref = "91.413"),
  ...
  fun expectedRecordIsInTheTopFive() {
    ...
    val failures = cases.mapNotNull { (query, expected) ->
      val top = engine.search(logbook, Adapter, RecordFilter(query = query), LocalDate(2026, 9, 6)).take(5).map { it.item.id }
  ...
  fun exactReferenceOutranksBodyMention() {
    val engine = SearchEngineImpl()
    val top = engine.search(logbook, Adapter, RecordFilter(query = "91.413"), LocalDate(2026, 9, 6)).map { it.item.id }
    assertWithMessage(top.toString()).that(top.first()).isEqualTo("certs")
  }
}
