...
object Stemmer {
  fun stem(word: String): String = dropSilentE(
    when {
      ...
      word.length > 4 && (word.endsWith("xes") || word.endsWith("ses") || word.endsWith(
        "ches"
      ) || word.endsWith("shes")) -> word.dropLast(2)

      word.length > 3 && word.endsWith("s") && !word.endsWith("ss") -> word.dropLast(
        1
      )
  ...
  private fun dropSilentE(word: String) =
    if (word.length > 4 && word.endsWith("e")) word.dropLast(1) else word
}
