...
class SquawkQuickActionsTest {
  ...
  fun openSquawk_offersResolveThenDelete() {
    ...
    assertThat(actions.map { it.label }).containsExactly("Resolve", "Delete")
      .inOrder()
    assertThat(actions.map { it.tone })
      .containsExactly(SwipeActionTone.POSITIVE, SwipeActionTone.DESTRUCTIVE)
      .inOrder()
  }
  ...
  fun closedSquawk_offersDeleteOnly() {
    ...
    assertThat(actionsFor(SquawkStatus.ADDRESSED).map { it.label }).containsExactly(
      "Delete"
    )
    assertThat(actionsFor(SquawkStatus.DISMISSED).map { it.label }).containsExactly(
      "Delete"
    )
  }
