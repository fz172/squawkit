...
class SettingsViewModelTest {
   ...
   */
  @Test
  fun askingToDelete_fallsBackToThePhraseForAHiddenAppleAddress() =
    runTest(testDispatcher) {
      ...
      assertThat(viewModel.user.value.deletionChallenge).isEqualTo(
  ...
  @Test
  fun askingToDelete_fallsBackToThePhraseWithoutAnEmail() =
    runTest(testDispatcher) {
  ...
  fun notificationsRowState_grantedAndOn_isDefault() = runTest(testDispatcher) {
    ...
    assertThat(viewModel.user.value.notificationsRowState).isEqualTo(
  ...
  @Test
  fun notificationsRowState_permissionDenied_isBlocked() =
    runTest(testDispatcher) {
      every { notificationPermission.observe() } returns MutableStateFlow(
  ...
  @Test
  fun notificationsRowState_permissionUnsupported_isBlocked() =
    runTest(testDispatcher) {
      every { notificationPermission.observe() } returns MutableStateFlow(
  ...
  @Test
  fun notificationsRowState_deniedAndMasterSwitchOff_blockedWins() =
    runTest(testDispatcher) {
      every { notificationPermission.observe() } returns MutableStateFlow(
        PermissionState.DENIED
      )
  ...
  @Test
  fun notificationsRowState_prefsUnresolved_readsAsDefault() =
    runTest(testDispatcher) {
