...
class TemplateThingIdentifierLookup(
  ...
  override suspend fun identifierOf(thingId: ThingId): String? {
    val thing = fleet.loadThing(thingId.value).first() ?: return null
    val key = templates.forThingWithFallback(thing).spec_fields.firstOrNull { it.is_identifier }?.key
      ?: return null
    return thing.specValue(key).takeIf { it.isNotBlank() }
  }
}
