...
class DataLogAttachmentPickerViewModelTest {
  ...
  private fun viewModel() =
    DataLogAttachmentPickerViewModel(manager, auth, analytics, templates, thingId)

  private fun log(id: String) = DataLog(
    id = DataLogId(id),
    start = Instant.parse("2026-09-02T21:47:56Z").toWireInstant(),
  ...
  fun rowsLoadAndATapSelects() = runTest {
    ...
    assertThat(state.rows.map { it.id }).containsExactly(DataLogId("a"), DataLogId("b"))
  ...
  fun aProbableDuplicateWaitsForKeepBoth() = runTest {
    ...
    vm.upload(listOf(file))
    assertThat(vm.uiState.value.import?.progress).isEqualTo(ImportProgress.NeedsConfirmation(DataLogId("old")))
  ...
  fun aFailedUploadStaysUntilDismissed() = runTest {
    ...
    vm.upload(listOf(file))
    assertThat(vm.uiState.value.import?.progress).isEqualTo(ImportProgress.Failed(ImportFailure.PARSE_ERROR))
  ...
  fun anImportFromHereIsReportedAsTheAttachmentSource() = runTest {
    every { manager.observeOne(thingId, DataLogId("new")) } returns flowOf(log("new"))
    ...
    assertThat(analytics.events.single().second).containsEntry("source", "attachment")
  }
}
