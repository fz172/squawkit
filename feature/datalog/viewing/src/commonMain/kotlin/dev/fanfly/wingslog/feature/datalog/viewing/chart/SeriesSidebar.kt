...
fun SeriesSidebar(
  ...
  Column(modifier = modifier.fillMaxSize()) {
    TabRow(selectedTabIndex = tab.ordinal) {
      Tab(selected = tab == SidebarTab.SERIES, onClick = { onTab(SidebarTab.SERIES) }, text = { Text(stringResource(Res.string.data_log_sidebar_series)) })
...
private fun SeriesTab(
  ...
  LazyColumn(modifier = Modifier.fillMaxSize()) {
    items(plottable, key = { it.column }) { series ->
      SeriesRow(series, SeriesKey(series.column) in inTargetPane, onAdd, dragState, onDrop)
    }
  }
}
...
private fun InfoTab(facts: List<InfoFact>, identityMismatch: Boolean) {
  LazyColumn(modifier = Modifier.fillMaxSize(), contentPadding = androidx.compose.foundation.layout.PaddingValues(vertical = Spacing.small)) {
    if (identityMismatch) {
      item {
        StatusChip(
    ...
    items(facts, key = { it.label }) { fact ->
      Row(
        ...
        Text(fact.label, style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
