...
internal fun dataLogListViewModel(thingId: ThingId): DataLogListViewModel =
  koinViewModel(key = "datalog-${thingId.value}", parameters = { parametersOf(thingId.value) })
 ...
 */
@Composable
fun DataLogUploadFab(thingId: ThingId, onLinkAccount: () -> Unit, modifier: Modifier = Modifier) {
  val viewModel = dataLogListViewModel(thingId)
  val state by viewModel.uiState.collectAsStateWithLifecycle()
  var showPrompt by remember { mutableStateOf(false) }
  val pick = rememberFilePicker(onResult = viewModel::upload)
  ExtendedFloatingActionButton(
    onClick = { if (state.uploadGate == UploadGate.SignedIn) pick() else showPrompt = true },
    icon = { Icon(Icons.Filled.UploadFile, contentDescription = null) },
    text = { Text(stringResource(Res.string.data_log_upload, LexiconFormatter.titleCase(LocalThingLexicon.current.dataLogNoun))) },
