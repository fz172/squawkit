...
class NavigationTest {
  ...
  fun zoomInHalvesTheSpanAroundTheAnchor() {
    ...
    assertThat(Navigation.zoomAround(null, duration, 0.0, 4.0)).isEqualTo(
      ViewWindow(0, 900)
    )
  }
  ...
  fun zoomClampsToFiveSecondsAndToTheLog() {
    val tiny =
      Navigation.zoomAround(ViewWindow(100, 110), duration, 0.5, 100.0)!!
    assertThat(tiny.lengthSeconds).isEqualTo(Navigation.MIN_SPAN_SECONDS)
    assertThat(
      Navigation.zoomAround(
        ViewWindow(100, 110),
        duration,
        0.5,
        100.0
  ...
  fun panClampsAndIsANoOpAtFullZoomOut() {
    ...
    assertThat(Navigation.pan(ViewWindow(100, 200), duration, 50.0)).isEqualTo(
      ViewWindow(150, 250)
    )
    assertThat(
      Navigation.pan(
    ...
    ).isEqualTo(ViewWindow(0, 100))
    assertThat(
      Navigation.pan(
        ViewWindow(3400, 3500),
        duration,
        500.0
      )
    ).isEqualTo(ViewWindow(3500, 3600))
  }
  ...
  fun brushMapsPixelsToTimeInsideTheCurrentView() {
    ...
    ).isEqualTo(ViewWindow(1000, 1500))
  }

  @Test
  fun aTinyBrushExpandsToTheMinimumSpanAndAnEmptyOneChangesNothing() {
    val v =
      Navigation.brushToWindow(ViewWindow(0, 100), duration, 500f, 510f, 1000)!!
    assertThat(v.lengthSeconds).isEqualTo(Navigation.MIN_SPAN_SECONDS)
    assertThat((v.startSeconds + v.endSeconds) / 2).isWithin(1)
      .of(50)
    assertThat(
      Navigation.brushToWindow(
        ViewWindow(0, 100),
