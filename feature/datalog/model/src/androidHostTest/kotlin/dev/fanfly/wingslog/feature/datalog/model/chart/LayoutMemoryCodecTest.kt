...
class LayoutMemoryCodecTest {
  ...
  private fun catalogue(vararg columns: Int) = columns.map {
    DataLogSeries(column = it, kind = DataLogSeriesKind.DATA_LOG_SERIES_KIND_NUMERIC)
  }
  ...
  fun aRoundTripKeepsThePanesTheTargetAndTheAxis() {
    val encoded = LayoutMemoryCodec.encode(LayoutMemory(layout, clockAxis = true))
  ...
  fun columnsTheLogNoLongerHasAreDropped() {
    val encoded = LayoutMemoryCodec.encode(LayoutMemory(layout, clockAxis = false))
    ...
    assertThat(decoded.layout.panes.map { it.series }).containsExactly(listOf(SeriesKey(3)))
  ...
  fun nothingLeftOrNotOursDecodesToNull() {
    ...
    assertThat(LayoutMemoryCodec.decode("v2;c=1;t=0;p=3", catalogue(3))).isNull()
  ...
  fun panesAreRenumberedFromZeroSoTheIdsStayDense() {
    val sparse = ChartLayout(
      panes = listOf(Pane(PaneId(4), listOf(SeriesKey(3))), Pane(PaneId(9), listOf(SeriesKey(5)))),
      targetPane = PaneId(9),
    )
    ...
    assertThat(decoded.layout.panes.map { it.id }).containsExactly(PaneId(0), PaneId(1)).inOrder()
