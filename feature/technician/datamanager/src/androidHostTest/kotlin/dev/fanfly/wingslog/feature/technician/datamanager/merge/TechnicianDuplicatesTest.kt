...
class TechnicianDuplicatesTest {
  ...
  fun aHandTypedCopyOfYourself_isCaughtEvenWithNoCertificates() {
    ...
    val group = findDuplicates(
      manual = listOf(handTyped),
      mirrors = emptyList(),
      self = me
    )
      .single()
  ...
  fun theSelfRecord_isNeverAmongTheRowsToDelete() {
    ...
    val groups = findDuplicates(
      manual = listOf(handTyped),
      mirrors = listOf(member),
      self = me
    )

    assertThat(groups.flatMap { it.duplicates }
                 .map { it.id }).doesNotContain("self")
  }
  ...
  fun certificateTypesMatchRegardlessOfCaseOnLegacyRows() {
    ...
    val a =
      Technician(id = "m1", name = "Bob", cert_type = "certificate_type_amt")
    val b =
      Technician(id = "m2", name = "Bob", cert_type = "CERTIFICATE_TYPE_AMT")
  ...
  fun aManualRowMatchingBothAMemberAndAnotherManualRow_mergesIntoTheMember() {
    ...
    assertThat(groups.single().duplicates.map { it.id }).containsExactly(
      "m1",
      "m2"
    )
  }
