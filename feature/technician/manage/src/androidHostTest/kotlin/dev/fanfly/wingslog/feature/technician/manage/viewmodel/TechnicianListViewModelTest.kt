...
class TechnicianListViewModelTest {
  ...
  @Test
  fun linkedTechnicians_areKeptSeparateFromTheOwnList() =
    runTest(testDispatcher) {
      ...
      assertThat(state.technicians.map { it.id })
        ...
        .inOrder()
      assertThat(state.linkedTechnicians.map { it.source_uid }).containsExactly(
        LINKED_UID
      )
    }

  @Test
  fun linkedTechnicians_carrySourceUid_soTheyReadAsFirstParty() =
    runTest(testDispatcher) {
      every { sharingManager.observeLinkedTechnicians() } returns flowOf(
        listOf(
          Technician(
      ...
      val state =
        viewModel().uiState.first { it.linkedTechnicians.isNotEmpty() }
  ...
  @Test
  fun aManualRowLookingLikeAMember_promptsForReview() =
    runTest(testDispatcher) {
      every { technicianManager.observeTechnicians() } returns flowOf(
        listOf(
  ...
  @Test
  fun onceTheseDuplicatesAreReviewed_thePromptStopsForThem() =
    runTest(testDispatcher) {
      seedSpongeBobDuplicate()
      // Whatever signature the current duplicates produce, having reviewed exactly that set silences
      // the prompt.
      val current =
        viewModel().uiState.first { it.duplicates.isNotEmpty() }.duplicatesSignature
      every { technicianManager.observeReviewedDuplicatesSignature() } returns flowOf(
        current
      )
  ...
  @Test
  fun aStaleReviewMarker_doesNotMuteADifferentDuplicate() =
    runTest(testDispatcher) {
  ...
  @Test
  fun theSelfRecord_isNeverTheRowThatGetsMergedAway() =
    runTest(testDispatcher) {
      ...
      assertThat(state.duplicates.flatMap { it.duplicates }
                   .map { it.id })
