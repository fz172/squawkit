 ...
 *
 * Renders nothing at all when [offered] is empty — an account holding only Things whose templates
 * declare no credentials has no certification to offer, and a section header over an empty list
 * promises a control that is not coming.
 */
@Composable
fun CertificationInputFields(
  ...
  Column(
    ...
    // Only what the person does not already hold: a second A&P is not a thing anyone has.
    val addable = offered.filterNot { candidate -> entries.any { it.type == candidate.key } }
...
/**
 * @param offered null when no installed template declares this key — a shared technician carrying a
 *   credential from a preset this build lacks. The row still renders, under the raw key: dropping
 *   it would delete the person’s credential on the next save.
 */
@Composable
private fun CertificationRow(
  ...
  Column(verticalArrangement = Arrangement.spacedBy(Spacing.small)) {
    ...
    // `expires: false` REMOVES the date rather than greying it — an EPA 608 certification is issued
    // for life, so there is no date the user could be asked for. An unknown key keeps the row: the
    // stored expiry is the only thing this build knows about the credential.
    if (offered == null || offered.def.expires) {
      ExpirationRow(
        entry = entry,
...
private fun ExpirationRow(
  ...
  val dated = entry.expireLimit != CertExpireLimit.CERT_EXPIRE_LIMIT_NEVER_EXPIRES
  ...
  Column(verticalArrangement = Arrangement.spacedBy(Spacing.extraSmall)) {
    ...
    Row(
      ...
      FormValueField(
        value = if (dated) {
          entry.expiration?.toLocalDateTime(TimeZone.UTC)?.date?.toDisplayFormat().orEmpty()
        } else {
          ""
        },
