 ...
 * when it does, so the card renders with blank row labels and stays that way until some input event
 * forces a frame. Moving the mouse fixes it, which is not a thing to ask of someone looking at a
 * sign-in page.
 *
 ...
 * it names — **a label added to the card and not added here comes back as a blank row.**
 *
...
suspend fun warmLoginCardResources(environment: ResourceEnvironment) {
  val strings = listOf(
    ...
    Res.string.sign_in_section_label,
    Res.string.sign_in_connecting,
  ...
  // The provider marks are resources too, and miss the same way — a row whose icon is absent reads
  // as a broken row, not a loading one.
  for (drawable in listOf(
    Res.drawable.ic_google_rd_na,
