...
class AccountUpgradeNameTest {
  ...
  @Test
  fun upgrade_pushesTheInAppNameToTheAuthProfile_notTheOtherWayRound() =
    runTest(dispatcher) {
  ...
  fun upgrade_withNoInAppNameYet_pushesNothing() = runTest(dispatcher) {
    ...
    every { technicianManager.observeSelf() } returns flowOf(
      Technician(
        id = "self-1",
        name = ""
      )
  ...
  @Test
  fun merge_carriesTheWelcomeScreenNameOntoTheExistingAccount() =
    runTest(dispatcher) {
      ...
      every { authManager.getCurrentUser() } answers {
        currentUsers.removeFirstOrNull() ?: permanentUser
      }
      coEvery { authManager.upgradeAnonymousAccount(any()) } returns
  ...
  @Test
  fun merge_withNoWelcomeScreenName_leavesTheAccountNameAlone() =
    runTest(dispatcher) {
