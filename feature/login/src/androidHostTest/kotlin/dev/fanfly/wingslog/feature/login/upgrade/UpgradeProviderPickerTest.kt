...
class UpgradeProviderPickerTest {
  ...
  fun completions_emitOnceTheUpgradeSucceeds() = runTest(dispatcher) {
    ...
    every { authManager.getCurrentUser() } returnsMany listOf(
      guest,
      permanent,
      permanent,
      permanent
  ...
  @Test
  fun selectingEmail_asksForAnAddressInsteadOfStartingAProviderFlow() =
    runTest(dispatcher) {
  ...
  @Test
  fun sendEmailLink_stashesTheAddressAgainstTheGuestUid() =
    runTest(dispatcher) {
      coEvery { authManager.sendSignInLink(EMAIL) } returns SendLinkResult.Sent(
        EMAIL
      )
      val vm = viewModel().also { it.select(AuthProvider.Email) }
  ...
  @Test
  fun sendEmailLink_keepsTheTypedAddressWhenItIsRejected() =
    runTest(dispatcher) {
  ...
  @Test
  fun confirmingAnAppleMerge_reauthorizesRatherThanReplaying() =
    runTest(dispatcher) {
      coEvery { authManager.upgradeAnonymousAccount(AuthProvider.Apple) } returns
        AccountUpgradeResult.ReauthRequiredToMerge(AuthProvider.Apple)
  ...
  fun incomingLink_asksBeforeLinking() = runTest(dispatcher) {
    ...
    coVerify(exactly = 0) {
      authManager.completeUpgradeWithEmailLink(
        any(),
        any()
      )
    }
  ...
  @Test
  fun incomingLink_isLeftForTheSignInFlowWhenTheUserIsNotAGuest() =
    runTest(dispatcher) {
  ...
  @Test
  fun incomingLink_isIgnoredWhenThisSessionNeverRequestedOne() =
    runTest(dispatcher) {
