...
sealed interface ThingOverviewAction {
  ...
  data class OpenDataLogClick(val thingId: String, val dataLogId: DataLogId) :
    ThingOverviewAction
  ...
  data class SquawkResolveClick(val squawk: SquawkWithStatus) :
    ThingOverviewAction
  ...
  /** Detail sheet only: a dismissed squawk goes back to open. */
  data class SquawkReopenClick(val squawkId: String) : ThingOverviewAction
  data class ConfirmDismissSquawk(val reason: SquawkDismissReason) :
    ThingOverviewAction

  data object CancelDismissSquawk : ThingOverviewAction
  data class DeleteSquawkClick(val squawk: SquawkWithStatus) :
    ThingOverviewAction
  ...
  data class TaskResolveClick(val card: MaintenanceTaskWithStatus) :
    ThingOverviewAction
  ...
  data class TaskSkipClick(val card: MaintenanceTaskWithStatus) :
    ThingOverviewAction
  ...
  data class DeleteTaskClick(val card: MaintenanceTaskWithStatus) :
    ThingOverviewAction
}
