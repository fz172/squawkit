# Graph Report - squawkit  (2026-07-19)

## Corpus Check
- 726 files · ~500,978 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4848 nodes · 8206 edges · 495 communities (406 shown, 89 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 850 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e1d09417`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- Cloud Functions Auth & Callables
- Export File Sharing & History UI
- PDF Export Rendering
- Auth / Login Flow
- Logbook Export Archive Builder
- Maintenance Task Due Computation
- Technician Duplicate Resolution
- Maintenance Log Proto Codec
- Squawk Form UI
- Image Compression
- Attachment Form Controller
- Maintenance Log Form ViewModel
- Adaptive App Shell
- Export Delivery Email Service
- Local Blob Store (BlobId)
- Agent Config & Architecture Docs
- Sync Push Worker
- Export Manager
- Attachment Picker Sheet
- Sharing Manager
- Task Detail Tabs UI
- Maintenance Task Proto
- Blob Store (SQLDelight/Sha256)
- Aircraft Scope Resolver
- Blob GC / Record Deletion
- Navigation Screen Routes
- Attachment Manager Tests
- Firestore Pull Subscription
- Export ViewModel
- Web Landing Page
- AppCapability (platform gate)
- BlobRef Store
- Task Form ViewModel
- Tombstone GC
- Form Field Components
- Task DataManager
- Edit Aircraft ViewModel
- Shared Aircraft Ref Proto
- iOS Blob Upload (URLSession)
- Cloud Sync & Share Models
- Appearance / Theme Store
- Database Factory & Write Lock
- Status Chip & Due Metadata
- Overview / Dashboard Tab
- Blob Delete Worker
- Firebase Admin & Blob Broker
- Technician Proto
- Fake Data Generator
- Attachment Refs & CollectionKind
- Maintenance Log Manager
- Entity Codec
- Storage Quota Checker
- Pending Attachment Model
- Redeem Invite ViewModel
- Request Export Delivery
- Community 55
- Community 56
- Community 57
- Community 58
- Community 59
- Community 60
- Community 61
- Community 62
- Community 63
- Community 64
- Community 65
- Community 66
- Community 67
- Community 68
- Community 69
- Community 70
- Community 71
- Community 72
- Community 73
- Community 74
- Community 75
- Community 76
- Community 77
- Community 78
- Community 79
- Community 80
- Community 81
- Community 82
- Community 83
- Community 84
- Community 85
- Community 86
- Community 87
- Community 88
- Community 89
- Community 90
- Community 91
- Community 92
- Community 93
- Community 94
- Community 95
- Community 96
- Community 97
- Community 98
- Community 99
- Community 100
- Community 101
- Community 102
- Community 103
- Community 104
- Community 105
- Community 106
- Community 107
- Community 108
- Community 109
- Community 110
- Community 111
- Community 112
- Community 113
- Community 114
- Community 115
- Community 116
- Community 117
- Community 118
- Community 119
- Community 120
- Community 121
- Community 122
- Community 123
- Community 124
- Community 125
- Community 126
- Community 127
- Community 128
- Community 129
- Community 130
- Community 131
- Community 132
- Community 133
- Community 134
- Community 135
- Community 136
- Community 137
- Community 138
- Community 139
- Community 140
- Community 141
- Community 142
- Community 143
- Community 144
- Community 145
- Community 146
- Community 147
- Community 148
- Community 149
- Community 150
- Community 151
- Community 152
- Community 153
- Community 154
- Community 155
- Community 156
- Community 157
- Community 158
- Community 159
- Community 160
- Community 161
- Community 162
- Community 163
- Community 164
- Community 165
- Community 166
- Community 167
- Community 168
- Community 169
- Community 170
- Community 171
- Community 172
- Community 173
- Community 174
- Community 175
- Community 176
- Community 177
- Community 178
- Community 179
- Community 180
- Community 181
- Community 182
- Community 183
- Community 184
- Community 185
- Community 186
- Community 187
- Community 188
- Community 189
- Community 190
- Community 191
- Community 192
- Community 193
- Community 194
- Community 195
- Community 196
- Community 197
- Community 198
- Community 199
- Community 200
- Community 201
- Community 202
- Community 203
- Community 204
- Community 205
- Community 206
- Community 207
- Community 208
- Community 209
- Community 210
- Community 211
- Community 212
- Community 213
- Community 214
- Community 215
- Community 216
- Community 217
- Community 218
- Community 219
- Community 220
- Community 221
- Community 222
- Community 223
- Community 224
- Community 225
- Community 226
- Community 227
- Community 228
- Community 229
- Community 230
- Community 231
- Community 232
- Community 233
- Community 234
- Community 235
- Community 236
- Community 237
- Community 238
- Community 239
- Community 240
- Community 241
- Community 242
- Community 243
- Community 244
- Community 245
- Community 246
- Community 247
- Community 248
- Community 249
- Community 250
- Community 251
- Community 252
- Community 253
- Community 254
- Community 255
- Community 256
- Community 257
- Community 258
- Community 259
- Community 260
- Community 261
- Community 262
- Community 263
- Community 264
- Community 265
- Community 266
- Community 267
- Community 268
- Community 269
- Community 270
- Community 271
- Community 272
- Community 273
- Community 274
- Community 275
- Community 276
- Community 277
- Community 278
- Community 279
- Community 280
- Community 281
- Community 282
- Community 283
- Community 284
- Community 285
- Community 286
- Community 287
- Community 288
- Community 289
- Community 290
- Community 291
- Community 293
- Community 294
- Community 295
- Community 296
- Community 297
- Community 298
- Community 299
- Community 300
- Community 301
- Community 302
- Community 303
- Community 304
- Community 305
- Community 306
- Community 307
- Community 308
- Community 309
- Community 310
- Community 311
- Community 312
- Community 313
- Community 314
- Community 315
- Community 316
- Community 317
- Community 318
- Community 319
- Community 320
- Community 321
- Community 322
- Community 323
- Community 324
- Community 325
- Community 334
- Community 335
- Community 336
- Community 337
- Community 338
- Community 342
- Community 343
- Community 346
- Community 347
- Community 390
- Community 391
- SyncSettingsViewModel
- .setUp

## God Nodes (most connected - your core abstractions)
1. `BlobId` - 135 edges
2. `EntityScope` - 65 edges
3. `CollectionKind` - 47 edges
4. `LogbookExportArchiveBuilder` - 47 edges
5. `MaintenanceLogFormViewModel` - 46 edges
6. `Aircraft` - 39 edges
7. `SquawkFormViewModelTest` - 38 edges
8. `LocalFirstAttachmentManagerImplTest` - 36 edges
9. `SquawkFormViewModel` - 36 edges
10. `TaskDueManagerImplTest` - 34 edges

## Surprising Connections (you probably didn't know these)
- `SquawkIt design tokens (Aviation Blue, Space Grotesk)` --semantically_similar_to--> `SquawkIt Privacy Notice`  [INFERRED] [semantically similar]
  docs/web/promo_site_design.html → webApp/src/jsMain/resources/privacy.html
- `Play Store Icon 512px` --semantically_similar_to--> `iOS App Icon (Assets.xcassets AppIcon)`  [INFERRED] [semantically similar]
  docs/product/play_store_icon_512.png → iosApp/iosApp/Assets.xcassets/AppIcon.appiconset/gemini-svg (8).png
- `AppEntry()` --calls--> `WingslogTheme()`  [INFERRED]
  composeApp/src/commonMain/kotlin/dev/fanfly/wingslog/AppEntry.kt → core/ui/theme/src/commonMain/kotlin/dev/fanfly/wingslog/core/ui/theme/Theme.kt
- `AppEntry()` --calls--> `formDialogs()`  [INFERRED]
  composeApp/src/commonMain/kotlin/dev/fanfly/wingslog/AppEntry.kt → feature/shell/src/commonMain/kotlin/dev/fanfly/wingslog/feature/shell/ShellNavGraph.kt
- `AppEntry()` --calls--> `settingsDetailRoutes()`  [INFERRED]
  composeApp/src/commonMain/kotlin/dev/fanfly/wingslog/AppEntry.kt → feature/shell/src/commonMain/kotlin/dev/fanfly/wingslog/feature/shell/ShellNavGraph.kt

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **Local-first storage + sync engine data flow** — agents_entity_store, agents_sync_engine, agents_syncdocwire, agents_collection_kind, agents_wire_protobuf_models [EXTRACTED 0.90]
- **Firebase deploy pipeline (rules, functions, storage, web)** — github_workflows_deploy_firestore_rules_workflow, github_workflows_deploy_functions_workflow, github_workflows_deploy_storage_rules_workflow, github_workflows_deploy_web_workflow, github_workflows_firebase_project_wingslog_9ca4e [EXTRACTED 0.85]
- **Aviation design language (palette + typography + status rules)** — design_instrument_palette, design_typography_hierarchy, design_advisory_amber_rule, design_mono_rule, design_critical_status_hierarchy [EXTRACTED 0.90]
- **Guest account merge flow (link, migrate, sync)** — docs_account_account_upgrade_design_account_upgrade_result, docs_account_account_upgrade_design_local_account_migrator, docs_account_account_upgrade_design_entity_scope_user_root, docs_product_prd_sync_engine [EXTRACTED 0.90]
- **On-device export pipeline** — docs_export_export_logs_design_export_manager, docs_export_export_logs_design_logbook_export_aggregator, docs_export_export_logs_design_csv_writer, docs_export_export_logs_design_zip_file_writer, docs_export_export_logs_design_attachment_bundler [EXTRACTED 0.90]
- **Server-driven export email delivery** — docs_export_export_email_automation_design_export_delivery_manager, docs_export_export_email_automation_design_request_export_delivery, docs_export_export_email_automation_design_export_history_repository, docs_export_export_email_automation_design_email_address_resolver [EXTRACTED 0.85]
- **Proto sync engine push/pull/hydrate flow** — docs_storage_storage_r1_design_push_worker, docs_storage_storage_r1_design_pull_listener, docs_storage_storage_r1_design_hydration_runner, docs_storage_storage_mode_prd_lww [EXTRACTED 0.90]
- **R2 attachment blob pipeline** — docs_storage_storage_r2_design_local_blob_store, docs_storage_storage_r2_design_blob_upload_driver, docs_storage_storage_r2_design_blob_index_reconciler, docs_storage_storage_mode_prd_blob_object, docs_storage_storage_r2_design_remote_state [EXTRACTED 0.85]
- **Shared-aircraft attachment broker flow** — docs_sharing_aircraft_sharing_design_stream_blob, docs_sharing_aircraft_sharing_design_blob_upload_session, docs_sharing_aircraft_sharing_design_aircraft_shares_acl, docs_sharing_aircraft_sharing_design_authorizing_broker [EXTRACTED 0.85]
- **App icon assets rendering the SquawkIt airplane mark across platforms** — docs_branding_cloud_console_app_icon_120_png_icon, docs_branding_cloud_console_app_icon_120_svg_icon, docs_product_play_store_icon_512_png_icon, iosapp_iosapp_assets_xcassets_appicon_appiconset_gemini_svg_8_png_icon, webapp_src_jsmain_resources_apple_touch_icon_png_icon, webapp_src_jsmain_resources_favicon_192_png_favicon, webapp_src_jsmain_resources_og_image_png_og_image [INFERRED 0.85]
- **SquawkIt Core App Navigation (Dashboard, Squawks, Maint., Logs)** — docs_product_screenshots_airplane_overview_aircraft_dashboard, docs_product_screenshots_squawks_squawks, docs_product_screenshots_maintenance_tasks_maintenance_tasks, docs_product_screenshots_logs_logs_screen [INFERRED 0.85]
- **SquawkIt Play Store listing screenshot set** — docs_product_store_assets_play_store_screenshot_01_overview_fleet_overview_promo, docs_product_store_assets_play_store_screenshot_02_maintenance_tasks_maintenance_tasks_promo, docs_product_store_assets_play_store_screenshot_03_squawks_squawks_promo, docs_product_store_assets_play_store_screenshot_04_logs_maintenance_logs_promo, docs_product_store_assets_play_store_screenshot_05_work_logs_work_logs_promo, docs_product_store_assets_play_store_screenshot_06_export_export_logs_promo [INFERRED 0.75]

## Communities (495 total, 89 thin omitted)

### Community 0 - "Cloud Functions Auth & Callables"
Cohesion: 0.08
Nodes (41): adminDb, health_probe, HealthProbeResponse, ALLOWED_APP_IDS, CallableIdentity, requireAuthenticatedApp(), cancelAircraftShareInvite, CancelRequest (+33 more)

### Community 1 - "Export File Sharing & History UI"
Cohesion: 0.05
Nodes (42): AndroidExportFileSharer, rememberExportFileSharer(), ExportFileSharer, rememberExportFileSharer(), ExportHistoryRoute(), NavController, aircraftSummary(), deleteConfirmBody() (+34 more)

### Community 2 - "PDF Export Rendering"
Cohesion: 0.09
Nodes (17): AircraftPdfDocument, PdfSummaryCard, PdfSummaryRow, PdfSummarySection, PdfTableSection, AircraftPdfWriter, ByteArray, ByteArray (+9 more)

### Community 3 - "Auth / Login Flow"
Cohesion: 0.05
Nodes (33): AuthFlow(), AuthStep, EmailSignIn, Login, NameEntry, Welcome, FirebaseUser, ViewModel (+25 more)

### Community 4 - "Logbook Export Archive Builder"
Cohesion: 0.11
Nodes (10): AircraftBundle, AttachmentExportManifest, AircraftExport, Engine, MaintenanceLog, Technician, TimeZone, LogbookExportArchiveBuilder (+2 more)

### Community 5 - "Maintenance Task Due Computation"
Cohesion: 0.11
Nodes (13): ForceCompliedStatus, InspectionRule, endOfMonth(), MaintenanceLog, MaintenanceTask, TaskDueManagerImpl, MaintenanceLog, MaintenanceTask (+5 more)

### Community 6 - "Technician Duplicate Resolution"
Cohesion: 0.08
Nodes (21): certKey(), DuplicateGroup, DuplicateResolution, MERGE_INTO_MEMBER, MERGE_MANUAL, WARN_MIRROR_CONFLICT, findDuplicates(), Technician (+13 more)

### Community 7 - "Maintenance Log Proto Codec"
Cohesion: 0.07
Nodes (30): Attachment, ComponentType, componentTypeFromJSON(), componentTypeToJSON(), Builtin, DeepPartial, Exact, fromJsonTimestamp() (+22 more)

### Community 8 - "Squawk Form UI"
Cohesion: 0.05
Nodes (17): DismissSquawkDialog(), AddSquawkRoute(), NavController, EditSquawkRoute(), NavController, Modifier, SnackbarHostState, SquawkFormScreen() (+9 more)

### Community 9 - "Image Compression"
Cohesion: 0.07
Nodes (26): Bitmap, Blob, Data, toFirebaseData(), ImageCompressorImpl, ByteArray, ImageCompressor, isCompressiblePhotoMime() (+18 more)

### Community 10 - "Attachment Form Controller"
Cohesion: 0.08
Nodes (8): AddFileError, AttachmentFormController, Failed, FileTooLarge, StateFlow, FileTooLargeException, Exception, AttachmentFormControllerTest

### Community 11 - "Maintenance Log Form ViewModel"
Cohesion: 0.05
Nodes (7): FormSnapshot, MaintenanceLogFormUiState, Job, StateFlow, Technician, ViewModel, MaintenanceLogFormViewModel

### Community 12 - "Adaptive App Shell"
Cohesion: 0.09
Nodes (34): AdaptiveAppShell(), AdaptiveShellUiState, EmptyFleetScaffold(), EmptyFleetShell(), Modifier, SnackbarHostState, ShellAircraft, ShellForTier() (+26 more)

### Community 13 - "Export Delivery Email Service"
Cohesion: 0.10
Nodes (30): ExportDeliveryService, buildExportReadyHtml(), buildExportReadyText(), escapeHtml(), ExportMailer, ExportMailerInput, formatBytes(), formatDateTime() (+22 more)

### Community 14 - "Local Blob Store (BlobId)"
Cohesion: 0.08
Nodes (8): BlobId, ByteArray, Flow, Result, LocalBlobStore, TestClock, WingsLogDatabase, SqlDelightLocalBlobStoreTest

### Community 15 - "Agent Config & Architecture Docs"
Cohesion: 0.07
Nodes (38): feature-module-scaffolder agent, Canonical feature module layout, Feature module dependency rules, feature/inspection scaffolding reference, kmm-test-writer agent, Canonical Firestore mock chain, KMM unit test stack (MockK/Truth/JUnit4/coroutines-test), graphify skill trigger (+30 more)

### Community 16 - "Sync Push Worker"
Cohesion: 0.11
Nodes (12): DirtyRow, hostRootScope(), Flow, parseScopePath(), PushWorker, scopePrefixFor(), sharedAircraftIn(), sharedAircraftScopePrefix() (+4 more)

### Community 17 - "Export Manager"
Cohesion: 0.09
Nodes (16): ExportDeliveryOutcome, Failed, InProgress, Sent, Throttled, ExportManager, ExportRecord, Flow (+8 more)

### Community 18 - "Attachment Picker Sheet"
Cohesion: 0.09
Nodes (28): ImageVector, Modifier, SheetState, TextStyle, PickerActionButton(), PickerDoneButton(), PickerSectionHeader(), PickerSelectableRow() (+20 more)

### Community 19 - "Sharing Manager"
Cohesion: 0.14
Nodes (12): dedupedByOwner(), Flow, Result, ShareRole, Technician, PreviewRequest, RedeemRequest, SharingManagerImpl (+4 more)

### Community 20 - "Task Detail Tabs UI"
Cohesion: 0.08
Nodes (23): BottomButtons(), BottomButtonsPreview(), Modifier, DocumentationFields(), Modifier, MaintenanceLog, TaskDetailTab(), IdentityRadioItem() (+15 more)

### Community 21 - "Maintenance Task Proto"
Cohesion: 0.06
Nodes (13): Builtin, ComplianceType, DeepPartial, EngineHourRule, Exact, fromJsonTimestamp(), fromTimestamp(), ImmediateRule (+5 more)

### Community 22 - "Blob Store (SQLDelight/Sha256)"
Cohesion: 0.11
Nodes (8): Blob_object, IntegrityError, Exception, ByteArray, sha256Hex(), ByteArray, Result, SqlDelightLocalBlobStore

### Community 23 - "Aircraft Scope Resolver"
Cohesion: 0.10
Nodes (14): CollectionReference, AircraftScopeResolver, Flow, aircraftChild(), EntityScope, userRoot(), DocumentReference, AircraftScopeResolverImpl (+6 more)

### Community 24 - "Blob GC / Record Deletion"
Cohesion: 0.11
Nodes (27): AttachmentType, Timestamp, blobIds(), blobIdsInPayload(), schemaCanOwnBlobs(), blobsReferencedByLiveRecords(), LiveRefs, onRecordDeleted (+19 more)

### Community 25 - "Navigation Screen Routes"
Cohesion: 0.11
Nodes (20): AdaptiveShell, AddAircraft, AddMaintenanceLog, AddMaintenanceTask, AddSquawk, EditAircraft, EditMaintenanceLog, EditMaintenanceTask (+12 more)

### Community 27 - "Firestore Pull Subscription"
Cohesion: 0.11
Nodes (13): DocumentSnapshot, FirestorePullSubscription, Flow, decodeRemoteEntity(), FirestoreRemoteFetcher, RemoteEntity, Flow, PullSubscription (+5 more)

### Community 28 - "Export ViewModel"
Cohesion: 0.10
Nodes (12): ExportDeliveryEmailSource, AUTH_FALLBACK, EXPLICIT, ExportDeliveryInfo, AircraftOptionRow(), DateRangeSection(), RangePill(), AircraftSelectionRow (+4 more)

### Community 29 - "Web Landing Page"
Cohesion: 0.18
Nodes (28): Color, ImageVector, LandingColors, strokeIcon(), AuthButton(), BrandMark(), BrandWordmark(), CardGrid() (+20 more)

### Community 30 - "AppCapability (platform gate)"
Cohesion: 0.11
Nodes (11): createAppCapability(), AppCapability, createAppCapability(), createAppCapability(), createAppCapability(), SettingsUiState, Job, StateFlow (+3 more)

### Community 31 - "BlobRef Store"
Cohesion: 0.14
Nodes (6): BlobRef, Flow, ByteArray, Flow, Result, RecordingBlobStore

### Community 32 - "Task Form ViewModel"
Cohesion: 0.09
Nodes (11): MaintenanceTask, Job, MaintenanceLog, StateFlow, ViewModel, Loading, PickError, Success (+3 more)

### Community 33 - "Tombstone GC"
Cohesion: 0.16
Nodes (3): TombstoneGc, WingsLogDatabase, TombstoneGcTest

### Community 34 - "Form Field Components"
Cohesion: 0.11
Nodes (20): FormKeyboard, FormSectionLabel(), FormTextField(), FormValueField(), Color, Composable, Modifier, Shape (+12 more)

### Community 35 - "Task DataManager"
Cohesion: 0.11
Nodes (8): Flow, MaintenanceTask, Result, TaskDataManagerImpl, FakeScopeResolver, Flow, MaintenanceTask, TaskDataManagerImplTest

### Community 36 - "Edit Aircraft ViewModel"
Cohesion: 0.10
Nodes (4): EditAircraftViewModel, ViewModel, Propeller, PropellerBlade

### Community 37 - "Shared Aircraft Ref Proto"
Cohesion: 0.11
Nodes (11): Builtin, DeepPartial, Exact, KeysOfUnion, MessageFns, SharedAircraftRef, ShareRole, encodeSharedAircraftRef() (+3 more)

### Community 38 - "iOS Blob Upload (URLSession)"
Cohesion: 0.11
Nodes (9): BGProcessingTask, BlobUploadDelegate, NSError, NSObject, NSURLSession, NSURLSession, UrlSessionUploadScheduler, NSURLSessionTask (+1 more)

### Community 39 - "Cloud Sync & Share Models"
Cohesion: 0.16
Nodes (7): CloudSyncSetting, AircraftShareState, ShareMember, ShareRole, OWNER, TECHNICIAN, ManageAccessViewModelTest

### Community 40 - "Appearance / Theme Store"
Cohesion: 0.12
Nodes (10): AndroidAppearanceStore, AppearanceController, AppearanceMode, DARK, LIGHT, SYSTEM, AppearanceStore, StateFlow (+2 more)

### Community 41 - "Database Factory & Write Lock"
Cohesion: 0.13
Nodes (8): DatabaseWriteLock, T, EntityStoreFactory, createWingsLogDatabase(), SqlDriver, WingsLogDatabase, WingsLogDatabase, SyncEngineTest

### Community 42 - "Status Chip & Due Metadata"
Cohesion: 0.11
Nodes (20): Modifier, StatusChip(), DueStatus, COMPLIED, DUE_SOON, NORMAL, OVERDUE, Color (+12 more)

### Community 43 - "Overview / Dashboard Tab"
Cohesion: 0.18
Nodes (21): Modifier, LogOnboardingCard(), DashboardLowerGrid(), EmptyRailState(), ImageVector, MaintenanceLog, Modifier, LargeOverviewTab() (+13 more)

### Community 44 - "Blob Delete Worker"
Cohesion: 0.11
Nodes (9): BlobDeleteWorker, CoroutineWorker, KoinComponent, Result, BlobDeleteDriver, BlobDeleteDriverTest, WingsLogDatabase, FirebaseStorage (+1 more)

### Community 45 - "Firebase Admin & Blob Broker"
Cohesion: 0.15
Nodes (12): adminAuth, adminStorage, attachmentsEnabled(), authorizeBlobRead(), blobObjectPath(), isShareMember(), loadShare(), BlobResponse (+4 more)

### Community 46 - "Technician Proto"
Cohesion: 0.09
Nodes (9): Builtin, CertExpireLimit, CertificateType, DeepPartial, Exact, fromJsonTimestamp(), fromTimestamp(), KeysOfUnion (+1 more)

### Community 47 - "Fake Data Generator"
Cohesion: 0.16
Nodes (12): generateRandomId(), AircraftSpec, FakeDataGenerator, MaintenanceLog, MaintenanceTask, Squawk, Technician, LogTemplate (+4 more)

### Community 48 - "Attachment Refs & CollectionKind"
Cohesion: 0.14
Nodes (12): AttachmentRefs, ByteArray, CollectionKind, decode(), encode(), FeatureLab, fromWire(), MaintenanceLog (+4 more)

### Community 49 - "Maintenance Log Manager"
Cohesion: 0.14
Nodes (9): MaintenanceOverview, Flow, MaintenanceLog, Result, MaintenanceLogManagerImpl, FakeScopeResolver, Flow, MaintenanceLog (+1 more)

### Community 50 - "Entity Codec"
Cohesion: 0.13
Nodes (8): EntityCodec, ByteArray, T, WireCodec, EntityCodecRegistry, T, CollectionKindCoverageTest, ByteArray

### Community 51 - "Storage Quota Checker"
Cohesion: 0.11
Nodes (7): Flow, QuotaChecker, State, FixedClock, Clock, WingsLogDatabase, QuotaCheckerTest

### Community 52 - "Pending Attachment Model"
Cohesion: 0.14
Nodes (17): Local, LocalLink, PendingAttachment, PendingDelete, Saved, visible(), AttachmentFormSection(), AttachmentPickerOption() (+9 more)

### Community 53 - "Redeem Invite ViewModel"
Cohesion: 0.22
Nodes (4): RedeemOutcome, appCapability(), FirebaseUser, RedeemViewModelTest

### Community 54 - "Request Export Delivery"
Cohesion: 0.10
Nodes (10): deliveryService, parseRequest(), requestExportDelivery, Builtin, DeepPartial, Exact, KeysOfUnion, MessageFns (+2 more)

### Community 55 - "Community 55"
Cohesion: 0.19
Nodes (7): ExportRequest, AttachmentExportPayload, LogbookExportAggregatorTest, ByteArray, MaintenanceLog, Squawk, LogbookExportArchiveBuilderTest

### Community 56 - "Community 56"
Cohesion: 0.15
Nodes (8): FleetEntry, FleetManager, Flow, Result, FleetManagerImpl, Flow, Result, SharedAircraftRef

### Community 57 - "Community 57"
Cohesion: 0.10
Nodes (17): Composable, Modifier, LogSection(), MaintenanceTask, Modifier, Squawk, Technician, LogRecordsTab() (+9 more)

### Community 59 - "Community 59"
Cohesion: 0.10
Nodes (21): AppCapability singleton (core:appinfo), attachmentUploadEnabled feature flag, Dogfood builds / stresstest fake data generator, Logbook export + requestExportDelivery Cloud Function, FeatureLabManager / FeatureFlags, Local-first attachments (R2), App Check + allowed app ID enforcement, EXPORT_DELIVERY_API_KEY in Secret Manager (+13 more)

### Community 60 - "Community 60"
Cohesion: 0.10
Nodes (4): UploadScheduler, WorkManagerUploadScheduler, ForegroundUploadScheduler, NetworkType

### Community 62 - "Community 62"
Cohesion: 0.14
Nodes (4): LocalAccountMigrator, LocalAccountMigratorImpl, WingsLogDatabase, LocalAccountMigratorTest

### Community 63 - "Community 63"
Cohesion: 0.18
Nodes (3): StorageEntity, FleetManagerImplTest, SharedAircraftRef

### Community 64 - "Community 64"
Cohesion: 0.16
Nodes (5): Clock, TestClock, WingsLogDatabase, SqlDelightEntityStoreTest, TestClock

### Community 65 - "Community 65"
Cohesion: 0.12
Nodes (18): CancelInviteRequest, CreateInviteRequest, CreateInviteResponse, InviteWire, MemberCreateWire, MemberSelfUpdateWire, MemberWire, PreviewResponse (+10 more)

### Community 66 - "Community 66"
Cohesion: 0.14
Nodes (17): SquawkStatus, ADDRESSED, DISMISSED, OPEN, SquawkWithStatus, toWithStatus(), Modifier, PriorityBadge() (+9 more)

### Community 67 - "Community 67"
Cohesion: 0.17
Nodes (3): classifyPushFailure(), isPermissionDenied(), PushFailureClassifierTest

### Community 68 - "Community 68"
Cohesion: 0.11
Nodes (7): Builtin, DeepPartial, Exact, fromJsonTimestamp(), fromTimestamp(), KeysOfUnion, MessageFns

### Community 69 - "Community 69"
Cohesion: 0.17
Nodes (6): AuthManagerImpl, AuthCredential, FirebaseUser, Credential, CredentialManager, GoogleIdTokenCredential

### Community 70 - "Community 70"
Cohesion: 0.15
Nodes (9): Modifier, LogStatsSection(), StatCell(), LogStats, AircraftOverviewViewModel, MaintenanceLog, StateFlow, ViewModel (+1 more)

### Community 71 - "Community 71"
Cohesion: 0.11
Nodes (9): FileByteReaderImpl, ByteArray, FileByteReader, ByteArray, FileByteReaderImpl, ByteArray, ByteArray, WebFileByteReader (+1 more)

### Community 72 - "Community 72"
Cohesion: 0.16
Nodes (17): AdvancedLinkedSection(), IntervalNumberInput(), ImageVector, MaintenanceTask, Modifier, StringResource, RecurrenceChoice(), TrackingModeButton() (+9 more)

### Community 73 - "Community 73"
Cohesion: 0.11
Nodes (6): AndroidAnalyticsPreferenceStore, AnalyticsPreferenceController, AnalyticsPreferenceStore, StateFlow, IosAnalyticsPreferenceStore, JsAnalyticsPreferenceStore

### Community 74 - "Community 74"
Cohesion: 0.17
Nodes (7): CoroutineScope, HydrationCounters, FirebaseUser, Job, StateFlow, WingsLogDatabase, SyncEngine

### Community 75 - "Community 75"
Cohesion: 0.11
Nodes (19): Authorizing broker vs signed URL, getBlobUploadSession broker, streamBlob authorizing proxy, onAircraftDeleted cascade trigger, Scheduled storageSweep, blob_object table, UploadScheduler (expect/actual), AttachmentManager (+11 more)

### Community 76 - "Community 76"
Cohesion: 0.17
Nodes (12): AuthorshipContext, Error, StateFlow, ViewModel, LinkTargets, Loaded, Loading, LogsLoadState (+4 more)

### Community 77 - "Community 77"
Cohesion: 0.18
Nodes (5): Flow, Result, ShareRole, Technician, SharingManager

### Community 78 - "Community 78"
Cohesion: 0.21
Nodes (4): Flow, Result, Technician, TechnicianManagerImpl

### Community 79 - "Community 79"
Cohesion: 0.14
Nodes (4): Continuation, AuthManagerImpl, IosGoogleSignInBridge, FirebaseUser

### Community 80 - "Community 80"
Cohesion: 0.17
Nodes (4): Aircraft, AircraftOverviewGatingTest, ShareRole, LogbookExportTable

### Community 81 - "Community 81"
Cohesion: 0.12
Nodes (13): AdaptiveCardList(), Dp, Modifier, T, DualSegmentedFilter(), Modifier, ComplianceSection(), Modifier (+5 more)

### Community 82 - "Community 82"
Cohesion: 0.13
Nodes (14): EmptyState(), ImageVector, Modifier, ActiveElsewhereScreen(), CompletionState, Failed, NeedEmail, Success (+6 more)

### Community 83 - "Community 83"
Cohesion: 0.14
Nodes (15): Density, Color, ImageVector, Shape, ResolveMenuItem(), ResolveMenuPositionProvider, ResolveOptionsMenu(), SpeechBubbleShape (+7 more)

### Community 84 - "Community 84"
Cohesion: 0.22
Nodes (17): AddLogClick, AddSquawkClick, AddTaskClick, AircraftOverviewAction, BackClick, CancelDeleteTask, ConfirmDeleteTask, DeleteConfirm (+9 more)

### Community 85 - "Community 85"
Cohesion: 0.21
Nodes (10): ExportHistoryRemoteRepository, ExportRecordAircraftWire, ExportRecordDateRangeWire, ExportRecordWire, ByteArray, ExportRecord, nullIfBlank(), toExportRecord() (+2 more)

### Community 86 - "Community 86"
Cohesion: 0.24
Nodes (3): XlsxSheet, ByteArray, XlsxWorkbookWriter

### Community 87 - "Community 87"
Cohesion: 0.22
Nodes (17): aircraftSummary(), CombinedRangeField(), DateRangePickerHeadline(), DateRangePickerHeadlineCell(), DeliveryFailure, DeliveryFailureSection(), EmailedSubtitle(), ExportBottomBar() (+9 more)

### Community 88 - "Community 88"
Cohesion: 0.16
Nodes (4): FakeScopeResolver, Flow, Squawk, SquawkManagerImplTest

### Community 89 - "Community 89"
Cohesion: 0.21
Nodes (4): PullListener, ByteArray, WingsLogDatabase, PullListenerTest

### Community 90 - "Community 90"
Cohesion: 0.29
Nodes (10): displayText(), HeroCaption(), Color, ImageVector, NavController, StatusRow(), StatusSection(), SyncSettingsScreen() (+2 more)

### Community 91 - "Community 91"
Cohesion: 0.16
Nodes (14): ShellSection, DASHBOARD, LOGS, SETTINGS, SQUAWKS, TASKS, AircraftSectionContent(), NavController (+6 more)

### Community 92 - "Community 92"
Cohesion: 0.13
Nodes (11): DashedButton(), Dp, Modifier, UnsavedChangesDialog(), AirframeSection(), EngineSection(), Engine, EditAircraftScreen() (+3 more)

### Community 93 - "Community 93"
Cohesion: 0.15
Nodes (6): Flow, StateFlow, SyncPreferences, FirebaseUser, WingsLogDatabase, SyncPreferencesTest

### Community 94 - "Community 94"
Cohesion: 0.21
Nodes (12): AdjSectionLabel(), formatEngineHours(), AnnotatedString, Modifier, monoOn(), relativeDaysAgoPhrase(), relativeDaysPhrase(), relativeEngineAgoPhrase() (+4 more)

### Community 95 - "Community 95"
Cohesion: 0.22
Nodes (12): EXPORT_DELIVERY_API_KEY, EXPORT_DELIVERY_RESEND_COOLDOWN_SECONDS, ExportDeliveryConfig, orphanBlobGraceDays(), requiredBooleanEnv(), requiredEnv(), requiredNonNegativeIntEnv(), requiredSecretValue() (+4 more)

### Community 96 - "Community 96"
Cohesion: 0.12
Nodes (6): Builtin, DeepPartial, Exact, KeysOfUnion, MessageFns, RFC-3339

### Community 97 - "Community 97"
Cohesion: 0.14
Nodes (4): FirebaseAnalyticsManager, AnalyticsManager, NoOpAnalyticsManager, trackScreenViews()

### Community 98 - "Community 98"
Cohesion: 0.21
Nodes (3): UserInfo, Technician, TechnicianManagerImplTest

### Community 99 - "Community 99"
Cohesion: 0.22
Nodes (10): AdaptiveFormDialogFrame(), Modifier, formDialogs(), NavController, settingsDetailRoutes(), sharingRoutes(), NavController, registerStressTestRoutes() (+2 more)

### Community 100 - "Community 100"
Cohesion: 0.18
Nodes (13): constrainedContentWidth(), ConstrainedFloatingAction(), ConstrainedTopBar(), ContentWidth, Dp, Modifier, Modifier, Technician (+5 more)

### Community 101 - "Community 101"
Cohesion: 0.16
Nodes (10): DynamicString, StringRes, UiText, ComponentSection(), Modifier, ReadOnlyComponentField(), Modifier, LogWorkTab() (+2 more)

### Community 102 - "Community 102"
Cohesion: 0.12
Nodes (4): AndroidSelectedAircraftStore, SelectedAircraftStore, IosSelectedAircraftStore, JsSelectedAircraftStore

### Community 103 - "Community 103"
Cohesion: 0.12
Nodes (16): UnitPillSelect(), fromTask(), MaintenanceTask, ScheduleMode, HOURS, LINKED, TIME, ScheduleRecurrence (+8 more)

### Community 104 - "Community 104"
Cohesion: 0.29
Nodes (3): ScheduleState, advanceDatePastToday(), BannerHelpersTest

### Community 105 - "Community 105"
Cohesion: 0.23
Nodes (4): Flow, Result, Technician, TechnicianManager

### Community 106 - "Community 106"
Cohesion: 0.13
Nodes (15): devDependencies, firebase, firebase-functions-test, @firebase/rules-unit-testing, grpc-tools, ts-proto, typescript, vitest (+7 more)

### Community 107 - "Community 107"
Cohesion: 0.21
Nodes (9): decode(), encode(), fromWire(), LocalOnly, RemoteOnly, RemoteState, Synced, Uploading (+1 more)

### Community 109 - "Community 109"
Cohesion: 0.14
Nodes (4): DatabaseHealth, DatabaseIntegrityChecker, DatabaseIntegrityCheckerTest, WingsLogDatabase

### Community 110 - "Community 110"
Cohesion: 0.27
Nodes (13): GroupedCard(), GroupedCheckboxRow(), GroupedLeadingIconChip(), GroupedRow(), GroupedRowGroup(), GroupedRowGroupPreview(), Color, Composable (+5 more)

### Community 112 - "Community 112"
Cohesion: 0.25
Nodes (15): ConfiguringContent(), EmptyAircraftContent(), ErrorResult(), ExportSelectionScreen(), ExportSetupList(), Color, Dp, ImageVector (+7 more)

### Community 115 - "Community 115"
Cohesion: 0.30
Nodes (12): DueMetadata, MaintenanceTaskWithStatus, CriticalAlertItem(), CriticalAlertsSection(), Modifier, Modifier, previewCard(), PreviewTaskCardItemComplied() (+4 more)

### Community 116 - "Community 116"
Cohesion: 0.16
Nodes (10): EmojiFallbackProvider(), EmojiFallbackRegistrar(), initializeAppCheck(), ReCaptchaV3Provider, initializeApp(), createSqliteWorker(), initializeFirebase(), initializeFirebaseAppCheck() (+2 more)

### Community 117 - "Community 117"
Cohesion: 0.20
Nodes (7): MainActivity, Bundle, ComponentActivity, AttachmentOpenerAndroid, Flow, StateFlow, Intent

### Community 118 - "Community 118"
Cohesion: 0.29
Nodes (5): await, ByteArray, Uint8Array, OpfsBlobFilesystem, unsafeCast

### Community 119 - "Community 119"
Cohesion: 0.14
Nodes (13): compileOnSave, compilerOptions, esModuleInterop, module, moduleResolution, outDir, rootDir, skipLibCheck (+5 more)

### Community 120 - "Community 120"
Cohesion: 0.18
Nodes (3): AuthManagerImpl, AuthCredential, FirebaseUser

### Community 121 - "Community 121"
Cohesion: 0.23
Nodes (7): CentralDirectoryEntry, Crc32, ByteArray, LittleEndianByteSink, StoredZipArchive, UInt, UIntArray

### Community 122 - "Community 122"
Cohesion: 0.38
Nodes (13): deleteExport(), discoverArchives(), ensureDirectory(), indexPath(), ByteArray, ExportRecord, listExports(), readIndex() (+5 more)

### Community 125 - "Community 125"
Cohesion: 0.22
Nodes (3): InviteLink, PendingInvite, InviteSheetViewModelTest

### Community 126 - "Community 126"
Cohesion: 0.20
Nodes (3): HydrationRunnerTest, ByteArray, WingsLogDatabase

### Community 128 - "Community 128"
Cohesion: 0.15
Nodes (12): engines, node, main, name, private, scripts, build, deploy (+4 more)

### Community 129 - "Community 129"
Cohesion: 0.22
Nodes (10): createAircraftShareInvite, redeemAircraftShareInvite, fft, req(), sha256(), mintCode(), wrappedCancel, wrappedCreate (+2 more)

### Community 130 - "Community 130"
Cohesion: 0.17
Nodes (8): revokeAircraftShare, fft, seedInvite(), sha256(), wrappedDeleted, wrappedRedeem, wrappedRevoke, wrappedUpdateRole

### Community 131 - "Community 131"
Cohesion: 0.19
Nodes (11): AppEntry(), authGraph(), NavController, shellGraph(), IntegrityRecoveryDialog(), RedeemHost(), NavController, NavigateToLoginOnSignOut() (+3 more)

### Community 133 - "Community 133"
Cohesion: 0.24
Nodes (10): IconLabelTabRow(), IconLabelTabRowPreview(), IconLabelTabSpec, Modifier, Modifier, LogTabRow(), LogTabSpec, Modifier (+2 more)

### Community 134 - "Community 134"
Cohesion: 0.15
Nodes (13): Technician profile mirror, Duplicate technician merge/reconcile, CollectionKind.Squawk, CollectionKind sealed type, Technician Feature Design Doc, TechnicianPickerSheet, Technician snapshot into log, Technician entity (+5 more)

### Community 135 - "Community 135"
Cohesion: 0.22
Nodes (9): Done, Downloading, Failed, Exception, LegacyAttachment, OpenState, AttachmentOpenerIos, Flow (+1 more)

### Community 136 - "Community 136"
Cohesion: 0.27
Nodes (4): AttachmentOpenerWeb, ByteArray, Flow, StateFlow

### Community 138 - "Community 138"
Cohesion: 0.26
Nodes (4): AccountUpgradeViewModel, AuthCredential, StateFlow, ViewModel

### Community 139 - "Community 139"
Cohesion: 0.23
Nodes (11): NavController, ManageAccessRoute(), ConfirmDialog(), Modifier, ShareRole, ManageAccessScreen(), ManageAccessUiState, MemberActions() (+3 more)

### Community 140 - "Community 140"
Cohesion: 0.28
Nodes (12): AlreadyMember, Confirm, Failed, Hidden, InfoDialog(), ShareRole, NeedsSignIn, RedeemConfirmationSheet() (+4 more)

### Community 141 - "Community 141"
Cohesion: 0.15
Nodes (3): EditTechnicianUiState, EditTechnicianViewModel, ViewModel

### Community 142 - "Community 142"
Cohesion: 0.33
Nodes (11): androidx, AffectedTasksSection(), AuthorshipLine(), MaintenanceLog, MaintenanceTask, Modifier, Squawk, LinkedEntityRow() (+3 more)

### Community 143 - "Community 143"
Cohesion: 0.20
Nodes (6): Any, Bool, MainEntry, String, UIApplication, Void

### Community 144 - "Community 144"
Cohesion: 0.26
Nodes (6): AppDelegate, NativeGoogleSignInProvider, String, UIViewController, NSObject, UIApplicationDelegate

### Community 145 - "Community 145"
Cohesion: 0.29
Nodes (3): PostWriteHook, BlobIndexReconciler, BlobIndexReconcilerTest

### Community 146 - "Community 146"
Cohesion: 0.23
Nodes (11): ColorScheme, StatusColors, statusColorsFor(), StatusTier, BLOCKING, CAUTION, CRITICAL, NEUTRAL (+3 more)

### Community 147 - "Community 147"
Cohesion: 0.21
Nodes (12): Aircraft Overview Tab Refactor, MaintenanceLogListContent (stateless extraction), Attachments PRD, Attachment quota caps (15 MB / 1 GB / dedup), Attachment sync-state status icon (RemoteState), Export attachment bundling, Paper-logbook CSV tab layout, Logbook Export (PRD) (+4 more)

### Community 148 - "Community 148"
Cohesion: 0.18
Nodes (12): Aircraft Dashboard / Overview, Airplane Overview Screenshot (Dashboard), Export Logs Feature, Export Data Screenshot (Export Ready), Logs Screen, Logs Screenshot, Maintenance Tasks, Maintenance Tasks Screenshot (+4 more)

### Community 149 - "Community 149"
Cohesion: 0.17
Nodes (12): Aircraft Sharing Design Doc, writer_uid authorship attestation, Aircraft Sharing PRD, Capability invites (link/QR token), Dependability First principle, Manage Access view, Owner + Technician roles, Deletion Cascade & Storage GC Design (+4 more)

### Community 150 - "Community 150"
Cohesion: 0.32
Nodes (11): AircraftOverviewEvent, NavigateBack, NavigateToAddLog, NavigateToAddSquawk, NavigateToAddTask, NavigateToEditAircraft, NavigateToEditLog, NavigateToEditSquawk (+3 more)

### Community 151 - "Community 151"
Cohesion: 0.18
Nodes (5): ByteArray, ExportedFile, ExportFileStore, ByteArray, ExportRecord

### Community 152 - "Community 152"
Cohesion: 0.26
Nodes (4): ExportRecordManifest, ByteArray, ExportRecord, LocalArchiveRecord

### Community 153 - "Community 153"
Cohesion: 0.24
Nodes (6): AndroidLinkSharer, rememberLinkSharer(), LinkSharer, rememberLinkSharer(), IosLinkSharer, rememberLinkSharer()

### Community 154 - "Community 154"
Cohesion: 0.24
Nodes (10): InviteSheetRoute(), NavController, InviteDetail(), InviteSheetScreen(), InviteSheetUiState, Modifier, ShareRole, PendingInviteCard() (+2 more)

### Community 155 - "Community 155"
Cohesion: 0.29
Nodes (4): Flow, Result, Squawk, SquawkManagerImpl

### Community 156 - "Community 156"
Cohesion: 0.27
Nodes (4): Flow, Result, Squawk, SquawkManager

### Community 157 - "Community 157"
Cohesion: 0.20
Nodes (4): FirestoreSyncWriter, SyncDocWire, SyncWrite, SyncWriter

### Community 158 - "Community 158"
Cohesion: 0.26
Nodes (4): DiscardedWork, hostUidFromRoot(), SharedScopeJanitor, SyncPrefs

### Community 159 - "Community 159"
Cohesion: 0.25
Nodes (5): ClosedRange, MaintenanceLog, Technician, WireInstant, LogbookExportAggregator

### Community 160 - "Community 160"
Cohesion: 0.24
Nodes (7): ComposeApp, ContentView, ContentView, iosApp, Scene, SwiftUI, View

### Community 161 - "Community 161"
Cohesion: 0.22
Nodes (9): create, directory, js, options, getFileHandle, handlePromise, parent, parentDirectory (+1 more)

### Community 162 - "Community 162"
Cohesion: 0.18
Nodes (11): Aviation Synonym Map, SearchManager, AircraftScopeResolver, EntityCodec registry, EntityStore, SQLDelight store, EntityScope, FleetManager (+3 more)

### Community 163 - "Community 163"
Cohesion: 0.20
Nodes (11): 30-day retention window, Local TombstoneGc, Anonymous-to-permanent linkWithCredential upgrade, Last-Writer-Wins on Server Timestamp, Proto Sync Engine, Tombstone (soft delete), No local clock in ordering (server-timestamp only), PullListener (+3 more)

### Community 165 - "Community 165"
Cohesion: 0.40
Nodes (10): deleteExport(), ByteArray, ExportRecord, listExports(), readIndex(), saveRecord(), storageKey(), triggerDownload() (+2 more)

### Community 167 - "Community 167"
Cohesion: 0.29
Nodes (4): Flow, MaintenanceLog, Result, MaintenanceLogManager

### Community 168 - "Community 168"
Cohesion: 0.24
Nodes (6): ActiveFilterChip(), FilterSheetContent(), FilterTypeRow(), Modifier, MaintenanceLogListContent(), MaintenanceLog

### Community 169 - "Community 169"
Cohesion: 0.25
Nodes (9): Composable, Modifier, SettingsCard(), SettingsRowGroup(), SettingsRowGroupPreview(), Modifier, NavController, SettingsContent() (+1 more)

### Community 172 - "Community 172"
Cohesion: 0.18
Nodes (3): InviteSheetViewModel, ShareRole, ViewModel

### Community 173 - "Community 173"
Cohesion: 0.20
Nodes (5): AdaptiveShellViewModel, StateFlow, ViewModel, ChangesDiscarded, SyncNotice

### Community 176 - "Community 176"
Cohesion: 0.22
Nodes (6): WingsLogApplication, Application, initKoin(), stressTestKoinModules(), KoinAppDeclaration, Module

### Community 177 - "Community 177"
Cohesion: 0.29
Nodes (7): AccountUpgradeResult, Cancelled, CredentialInUse, Failed, Linked, AuthCredential, AuthCredential

### Community 178 - "Community 178"
Cohesion: 0.22
Nodes (3): BlobFilesystem, blobRelativePath(), ByteArray

### Community 179 - "Community 179"
Cohesion: 0.27
Nodes (4): EntityStore, Flow, T, T

### Community 180 - "Community 180"
Cohesion: 0.29
Nodes (4): fileSizeParts(), formatFileSize(), StringResource, FormatFileSizeTest

### Community 184 - "Community 184"
Cohesion: 0.29
Nodes (8): AttachmentRow(), displayDomain(), Modifier, mimeLabel(), subtitle(), typeIcon(), AttachmentSection(), Modifier

### Community 185 - "Community 185"
Cohesion: 0.31
Nodes (6): CameraPickerDelegate, NSObject, rememberCameraCapture(), UIImagePickerController, UIImagePickerControllerDelegateProtocol, UINavigationControllerDelegateProtocol

### Community 186 - "Community 186"
Cohesion: 0.24
Nodes (8): ExportFormat, CSV, PDF, XLSX, FormatSection(), Composable, Section(), SectionHeader()

### Community 187 - "Community 187"
Cohesion: 0.31
Nodes (9): HeaderCell(), HeaderRow(), MaintenanceLog, Modifier, LogRow(), MaintenanceLogTable(), primaryTableHours(), LazyListState (+1 more)

### Community 188 - "Community 188"
Cohesion: 0.40
Nodes (9): AppearanceLabels(), AppearanceLeadingIcon(), AppearanceOption(), AppearanceSettingRow(), AppearanceToggle(), icon(), ImageVector, Modifier (+1 more)

### Community 189 - "Community 189"
Cohesion: 0.27
Nodes (4): AircraftShareDeepLinks, StateFlow, formatInviteCode(), normalizeInviteCode()

### Community 190 - "Community 190"
Cohesion: 0.20
Nodes (3): StateFlow, ViewModel, StressTestViewModel

### Community 192 - "Community 192"
Cohesion: 0.24
Nodes (3): AnalyticsSyncTelemetry, NoOp, SyncTelemetry

### Community 193 - "Community 193"
Cohesion: 0.42
Nodes (3): Flow, T, SqlDelightEntityStore

### Community 194 - "Community 194"
Cohesion: 0.25
Nodes (6): ColorScheme, platformColorScheme(), WingslogTheme(), Modifier, SyncHeroIllustration(), SyncHeroIllustrationInactivePreview()

### Community 195 - "Community 195"
Cohesion: 0.33
Nodes (9): Cloud Console App Icon 120px (PNG), Cloud Console App Icon 120 (SVG source), Play Store Icon 512px, iOS App Icon (Assets.xcassets AppIcon), SquawkIt Cyan-to-Violet Gradient Palette (#00E5FF -> #651FFF), SquawkIt Airplane Logo Mark, Web Apple Touch Icon, Web Favicon 192px (+1 more)

### Community 196 - "Community 196"
Cohesion: 0.25
Nodes (9): Local-First Storage PRD, R1 Design: Local-First Protos, HydrationRunner, sync_cursor table, R2 Design: Local-First Attachments, Hydration race bootstrap fix, Web Attachments Design, Web Target Expansion Plan (+1 more)

### Community 197 - "Community 197"
Cohesion: 0.25
Nodes (9): Local-First Architecture, SquawkIt Promo Site Design, SquawkIt design tokens (Aviation Blue, Space Grotesk), SEO / structured data requirements, Export README Template, Export archive layout (per-aircraft ZIP), Account Deletion Request redirect, webApp index.html (+1 more)

### Community 198 - "Community 198"
Cohesion: 0.28
Nodes (6): AircraftDataCard(), ComponentCard(), Composable, BladeChipsOverview(), EngineDetails(), Engine

### Community 201 - "Community 201"
Cohesion: 0.31
Nodes (6): FeatureLabManagerImpl, Flow, Result, toFeatureFlags(), toProto(), FeatureLabSettings

### Community 202 - "Community 202"
Cohesion: 0.42
Nodes (8): BadgeScheme, badgeSchemeFor(), ComponentTypeBadge(), MaintenanceLog, Modifier, MaintenanceLogCard(), PreviewMaintenanceLogCard(), primaryHours()

### Community 203 - "Community 203"
Cohesion: 0.22
Nodes (3): ShareRole, ViewModel, ManageAccessViewModel

### Community 204 - "Community 204"
Cohesion: 0.22
Nodes (5): BlobDownloadWorker, CoroutineWorker, KoinComponent, Result, BlobDownloadDriver

### Community 205 - "Community 205"
Cohesion: 0.22
Nodes (5): BlobUploadWorker, CoroutineWorker, KoinComponent, Result, BlobUploadDriver

### Community 206 - "Community 206"
Cohesion: 0.33
Nodes (4): Flow, MaintenanceTask, Result, TaskDataManager

### Community 207 - "Community 207"
Cohesion: 0.32
Nodes (5): UIViewController, ComposeView, Context, UIViewController, UIViewControllerRepresentable

### Community 208 - "Community 208"
Cohesion: 0.29
Nodes (3): actionCodeSettings(), EmailLinkAuthenticator, FirebaseUser

### Community 209 - "Community 209"
Cohesion: 0.39
Nodes (5): Allowed, DuplicateOnParent, PerParentExceeded, PerUserExceeded, QuotaResult

### Community 211 - "Community 211"
Cohesion: 0.29
Nodes (7): AnnotatedString, Modifier, PreviewBanner(), PreviewBannerTone, Active, Neutral, Warn

### Community 212 - "Community 212"
Cohesion: 0.29
Nodes (6): WingsLogTopAppBar(), FeatureLabScreen(), FeatureToggleRow(), ImageVector, NavController, TopAppBarScrollBehavior

### Community 214 - "Community 214"
Cohesion: 0.36
Nodes (7): FontFamily, TextStyle, rememberBrandHeadlineFamily(), rememberJetBrainsMonoFamily(), rememberWingslogTypography(), WingslogTypography, Typography

### Community 215 - "Community 215"
Cohesion: 0.25
Nodes (8): aircraft_shares ACL tree, attachmentsEnabled entitlement projection, Data stays under hosting owner + ACL grant, redeemAircraftShareInvite Cloud Function, Sharing security rules (role get()), SharedAircraftRef synced kind, SharedScopeJanitor purge, SharingManager

### Community 216 - "Community 216"
Cohesion: 0.29
Nodes (8): User Squawking Design Doc, addressed_by_log_id linkage, AogAlertSection, DualSegmentedFilter, Squawk, SquawkManager, SquawkStatus (Open/Addressed/Dismissed), User Squawking Support PRD

### Community 219 - "Community 219"
Cohesion: 0.46
Nodes (7): BlobSyncState, Downloading, PendingUpload, RemoteOnly, Synced, UploadFailed, Uploading

### Community 220 - "Community 220"
Cohesion: 0.25
Nodes (7): ExportProgressStep, BUILDING_ARCHIVE, COLLECTING_DATA, COMPRESSING_ARCHIVE, SAVING_FILE, UPLOADING_ARCHIVE, exportRunningPhases()

### Community 223 - "Community 223"
Cohesion: 0.32
Nodes (6): data, js, rememberLinkSharer(), shareData(), WebLinkSharer, url

### Community 224 - "Community 224"
Cohesion: 0.43
Nodes (7): ConfigSection(), displayText(), NavController, SliderRow(), StepperRow(), StressTestScreen(), IntRange

### Community 225 - "Community 225"
Cohesion: 0.43
Nodes (6): Done, Error, Idle, Running, StressTestState, StressTestSummary

### Community 226 - "Community 226"
Cohesion: 0.25
Nodes (8): StressTestProgressStep, CreatingAircraft, CreatingLog, CreatingSquawk, CreatingTask, CreatingTechnician, DismissingSquawk, MarkingSquawkAddressed

### Community 229 - "Community 229"
Cohesion: 0.36
Nodes (4): ResourceReader, ByteArray, ProvideWholeFileResourceReader(), WholeFileResourceReader

### Community 230 - "Community 230"
Cohesion: 0.29
Nodes (7): dependencies, @bufbuild/protobuf, firebase-admin, firebase-functions, @bufbuild/protobuf, firebase-admin, firebase-functions

### Community 233 - "Community 233"
Cohesion: 0.33
Nodes (5): TimeZone, WireInstant, toInstant(), toLocalDate(), toWireInstant()

### Community 235 - "Community 235"
Cohesion: 0.48
Nodes (6): DetailBody(), DetailEndDrawer(), DetailSheet(), Composable, Modifier, SheetState

### Community 236 - "Community 236"
Cohesion: 0.38
Nodes (7): AttachmentManager interface, Attachment protobuf message, Attachments Design (pre-local-first, superseded), Embed attachment metadata in parent proto, FilePicker / AttachmentOpener expect-actual, AttachmentFormController (shared), AttachmentBundler

### Community 237 - "Community 237"
Cohesion: 0.38
Nodes (7): backend/firebase Functions workspace, EmailAddressResolver, ExportDeliveryManager, Export Email Automation (Design), ExportHistoryRepository (account-scoped manifest), requestExportDelivery Cloud Function, Signed download link email delivery

### Community 238 - "Community 238"
Cohesion: 0.38
Nodes (7): Intelligent Search Implementation, FTS5 Trigram Tokenizer, MediaPipe Text Embedder (all-MiniLM-L6-v2), NLEmbedding (Apple Neural Engine), Reciprocal Rank Fusion, sqlite-vec extension, Vector Embeddings

### Community 239 - "Community 239"
Cohesion: 0.52
Nodes (6): AttachmentStatus, Failed, LocalOnly, RemoteOnly, Synced, Uploading

### Community 242 - "Community 242"
Cohesion: 0.38
Nodes (5): FleetEmptyState(), Modifier, AdaptiveShellRoute(), NavController, SettingsSection()

### Community 243 - "Community 243"
Cohesion: 0.67
Nodes (6): Assigned, authorship(), LogAuthorship, SelfSigned, Unknown, Unverifiable

### Community 244 - "Community 244"
Cohesion: 0.52
Nodes (6): DeleteSuccess, FileAdded, LinkAdded, MaintenanceLogFormEvent, PickError, SaveSuccess

### Community 245 - "Community 245"
Cohesion: 0.43
Nodes (5): Error, Loading, LogFilter, MaintenanceLogListUiState, Success

### Community 246 - "Community 246"
Cohesion: 0.38
Nodes (6): ImageVector, SettingsLevel, DANGER, DEFAULT, SettingsRow(), SettingsRowPreview()

### Community 247 - "Community 247"
Cohesion: 0.29
Nodes (3): EnterInviteCodeRoute(), NavController, EnterInviteCodeScreen()

### Community 249 - "Community 249"
Cohesion: 0.38
Nodes (3): BrowserTitleAnalytics, prettifyScreenName(), webPageTitle()

### Community 250 - "Community 250"
Cohesion: 0.33
Nodes (3): App, iosApp, Scene

### Community 251 - "Community 251"
Cohesion: 0.53
Nodes (4): Failed, InvalidEmail, SendLinkResult, Sent

### Community 252 - "Community 252"
Cohesion: 0.47
Nodes (4): getAuth(), GoogleAuthProvider, Promise, signInWithPopup()

### Community 253 - "Community 253"
Cohesion: 0.60
Nodes (5): ByteArray, rotateRight(), sha256Hex(), sha256Padded(), toHexWord()

### Community 254 - "Community 254"
Cohesion: 0.40
Nodes (6): AccountUpgradeResult sealed type, EntityScope.userRoot(uid) join key, Guest → Account Upgrade (Technical Design), Launch-time UID/scope reconciliation, Firebase linkWithCredential (UID preservation), LocalAccountMigrator (re-key local data)

### Community 255 - "Community 255"
Cohesion: 0.40
Nodes (6): CsvWriter (RFC 4180), ExportManager interface, Logbook Export (Design), LogbookExportAggregator, ZipFileWriter (expect/actual), Local-first storage architecture (R1/R2)

### Community 256 - "Community 256"
Cohesion: 0.33
Nodes (6): Adaptive Layout Redesign, AdaptiveAppShell, AdaptiveDetailContainer, Ambient aircraft selection (switcher), LayoutTier window-size model, NavigationSuiteScaffold

### Community 257 - "Community 257"
Cohesion: 0.53
Nodes (5): grantCameraUriPermission(), Context, rememberCameraCapture(), toFileProviderUri(), Uri

### Community 258 - "Community 258"
Cohesion: 0.47
Nodes (4): ExportDeliveryBackend, ExportDeliveryResult, RequestExportDeliveryRequestData, RequestExportDeliveryResponseData

### Community 260 - "Community 260"
Cohesion: 0.60
Nodes (5): Configuring, Error, ExportUiState, Running, Success

### Community 261 - "Community 261"
Cohesion: 0.33
Nodes (3): FeatureLabManager, Flow, Result

### Community 262 - "Community 262"
Cohesion: 0.33
Nodes (3): FeatureLabViewModel, StateFlow, ViewModel

### Community 263 - "Community 263"
Cohesion: 0.60
Nodes (5): Error, Idle, Success, UpgradeUiState, Working

### Community 264 - "Community 264"
Cohesion: 0.40
Nodes (4): FirebaseAuth, FirebaseCore, GoogleSignIn, UIKit

### Community 267 - "Community 267"
Cohesion: 0.70
Nodes (4): FloatingNavItem, FloatingPillNavigationBar(), Modifier, PillItem()

### Community 268 - "Community 268"
Cohesion: 0.50
Nodes (5): Guest → Account Upgrade (PRD), Merge Guest Data on Account Collision, Platform Feature Parity Matrix, Web requires permanent account (no guest), Feature Lab flag gating (FeatureLabManager)

### Community 269 - "Community 269"
Cohesion: 0.50
Nodes (5): ActionCodeSettings continue URL, App Links / Universal Links deep-link routing, Passwordless Email-Link Sign-In (Design), Shared EmailSignInScreen, Web SingleTabGate / OPFS completion constraint

### Community 270 - "Community 270"
Cohesion: 0.50
Nodes (5): AnalyticsManager (core:analytics), login_state user property, MissingClickAnalytics Lint rule, Three-feeder page-view tracking, Product Analytics (Firebase Analytics / GA4)

### Community 271 - "Community 271"
Cohesion: 0.50
Nodes (5): Codebase Cleanup Plan, feature:shell shared nav graph module, LocalBlobStore moved to core:storage.blob, Feature managers never touch Firestore, Firestore sync engine (feature/sync)

### Community 272 - "Community 272"
Cohesion: 0.40
Nodes (3): AttachmentOpener, Flow, StateFlow

### Community 273 - "Community 273"
Cohesion: 0.40
Nodes (4): Promise, getDirectory, rootPromise, storage

### Community 274 - "Community 274"
Cohesion: 0.70
Nodes (4): Done, Downloading, DownloadState, Failed

### Community 275 - "Community 275"
Cohesion: 0.70
Nodes (4): ByteArray, readFileBytes(), rememberFilePicker(), toByteArray()

### Community 276 - "Community 276"
Cohesion: 0.70
Nodes (4): AllTime, Custom, ExportDateRange, LastNMonths

### Community 277 - "Community 277"
Cohesion: 0.40
Nodes (4): ExportDisplayLocation, DOWNLOADS_SQUAWKIT, FILES_SQUAWKIT, UNKNOWN

### Community 279 - "Community 279"
Cohesion: 0.40
Nodes (4): DateRangeOption, AllTime, Custom, Last12Months

### Community 286 - "Community 286"
Cohesion: 0.40
Nodes (4): UserStatus, LOADING, LOGGED_OUT, UNKNOWN

### Community 287 - "Community 287"
Cohesion: 0.60
Nodes (4): AogAlertSection(), AogSquawkRow(), Modifier, Squawk

### Community 288 - "Community 288"
Cohesion: 0.70
Nodes (4): Done, HydrationState, Idle, InProgress

### Community 289 - "Community 289"
Cohesion: 0.70
Nodes (4): AuthExpired, Hydration, Push, SyncFailure

### Community 290 - "Community 290"
Cohesion: 0.60
Nodes (3): ComposeView, Context, UIViewController

### Community 291 - "Community 291"
Cohesion: 0.50
Nodes (3): exec(), onMessage(), ready

### Community 297 - "Community 297"
Cohesion: 0.50
Nodes (4): Play Store Promo: Your Fleet, At a Glance (Fleet Overview), Play Store Promo: Never Miss A Deadline (Maintenance Tasks), Fleet Overview / Dashboard, Maintenance Tasks

### Community 298 - "Community 298"
Cohesion: 0.67
Nodes (4): Play Store Promo: Every Entry, Fully Documented (Maintenance Logs), Play Store Promo: Every Task, On the Record (Work Logs), Maintenance Logs, Work Logs

### Community 305 - "Community 305"
Cohesion: 0.83
Nodes (3): gradlew script, die(), warn()

### Community 321 - "Community 321"
Cohesion: 0.67
Nodes (3): Screenshot Generator HTML Template, Play Store Screenshot Generator README, No per-user data on screenshot canvas

### Community 493 - "SyncSettingsViewModel"
Cohesion: 0.33
Nodes (4): StateFlow, ViewModel, SyncSettingsUiState, SyncSettingsViewModel

## Knowledge Gaps
- **307 isolated node(s):** `no-bare-koin-get.sh script`, `no-escape-chars.sh script`, `private`, `main`, `node` (+302 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **89 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `BlobId` connect `Local Blob Store (BlobId)` to `Community 135`, `Community 136`, `Community 145`, `Blob Store (SQLDelight/Sha256)`, `Attachment Manager Tests`, `BlobRef Store`, `Tombstone GC`, `iOS Blob Upload (URLSession)`, `Blob Delete Worker`, `Community 175`, `Attachment Refs & CollectionKind`, `Community 178`, `Storage Quota Checker`, `Community 60`, `Community 74`, `Community 204`, `Community 205`, `Community 111`, `Community 241`, `Community 117`?**
  _High betweenness centrality (0.147) - this node is a cross-community bridge._
- **Why does `Attachment` connect `Maintenance Log Proto Codec` to `Blob GC / Record Deletion`, `Community 68`, `Maintenance Task Proto`, `Community 111`?**
  _High betweenness centrality (0.131) - this node is a cross-community bridge._
- **Why does `Aircraft` connect `Community 80` to `Community 64`, `Tombstone GC`, `Edit Aircraft ViewModel`, `Logbook Export Archive Builder`, `Community 198`, `Community 199`, `Community 101`, `Fake Data Generator`, `Attachment Refs & CollectionKind`, `Community 113`, `Community 55`, `Community 56`, `Community 92`, `Community 63`?**
  _High betweenness centrality (0.124) - this node is a cross-community bridge._
- **Are the 57 inferred relationships involving `BlobId` (e.g. with `.open()` and `.onEntityWritten()`) actually correct?**
  _`BlobId` has 57 INFERRED edges - model-reasoned connections that need verification._
- **Are the 5 inferred relationships involving `LogbookExportArchiveBuilder` (e.g. with `.buildEntries_embedsAttachmentPayloadsAndLinksCsvRows()` and `.buildEntries_multiAircraftUsesOneFolderPerAircraftAndRootReadme()`) actually correct?**
  _`LogbookExportArchiveBuilder` has 5 INFERRED edges - model-reasoned connections that need verification._
- **What connects `no-bare-koin-get.sh script`, `no-escape-chars.sh script`, `private` to the rest of the system?**
  _307 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Cloud Functions Auth & Callables` be split into smaller, more focused modules?**
  _Cohesion score 0.07526881720430108 - nodes in this community are weakly interconnected._