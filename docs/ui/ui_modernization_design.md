...
| Motion | repo-wide | Zero `enterTransition`/`exitTransition` on any nav destination. Zero `Crossfade`, `AnimatedContent`, `animateContentSize`, `animateItem`, `SharedTransitionLayout`. Section switching is a bare state swap (`AdaptiveAppShell.kt:318`). `PRODUCT.md` asks for "pages turning, not views teleporting". |
| Form fields | `FormField.kt:86`, `:232`, `:113-114` | `FormTextField` uppercases its label. `FormValueField` renders a picker as label + plain text with no border, chevron or any tap affordance. One shared `required` string is the error message for every field in the app, and `supportingText` and error text share a slot. |
...
### UI-5 · Make pickers look like pickers

`FormValueField` (`FormField.kt:232`) gains a border, a chevron and a 48dp minimum height when it has
an `onClick`. Today `AIRFRAME` and `ROUTINE` render as bare bold text under a blue label, with
nothing saying they are tappable — and blue means link everywhere else in the app.

- **Ships alone:** a `core/ui` change that improves every picker in every form at once.
